    ...
    if ( ( fd = gdFsOpen( MltName, NULL ) ) != NULL ) {
      {
        ...
        err = sdMemBlkSetPrm( mem_blk, mlt, mlt_size, SDD_MEMBLK_SYNC_FUNC, NULL );
        err = sdMultiUnitDownload( mem_blk );
      }
      gdFsClose( fd );
    } else PrgStop();
  }
  err = sdMemBlkDestroy( mem_blk );
...
//  njPrint( NJM_LOCATION( x, y ), "Driver Version: %1d.%02x.%02x", ver.m_MajorVer, ver.m_MinorVer, ver.m_DebugVer );
  njPrint( NJM_LOCATION( x, y ), "Driver Version: %1d.%02x.%02x", ver.m_Major, ver.m_Minor, ver.m_Debug );
}
...
//  njPrint( NJM_LOCATION( x, y ), "Library Version: %1d.%02x.%02x", ver.m_MajorVer, ver.m_MinorVer, ver.m_DebugVer );
  njPrint( NJM_LOCATION( x, y ), "Library Version: %1d.%02x.%02x", ver.m_Major, ver.m_Minor, ver.m_Debug );
}
