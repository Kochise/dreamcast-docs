...
KMVOID AssertCallback(PKMASSERTIONFAIL pInfo)
{
	DPF("%s", pInfo->pszErrMsg);
	DPF("%s", pInfo->pszFormula);
	DPF("%s", pInfo->pszFile);
	DPF("Line = %d", pInfo->nLine);
}

Int UserInit(void)
{
	Kamui_Init();
	kmSetAssertCallback( AssertCallback );	/* Callback for Assert */
}
