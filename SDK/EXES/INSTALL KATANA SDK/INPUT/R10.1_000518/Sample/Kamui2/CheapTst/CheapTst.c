...
/********************************************************************************************/
/* Name:     CheapTst.c                                                                     */
/* Title:    CheapTest Kamui2 Example                                                       */
/* Author:   Gary Lake                                                                      */
/* Created:  June 4, 1999                                                                   */
/*                                                                                          */
/* Version:  1.0                                                                            */
...
/*                                                                                          */
/* Description:                                                                             */
/*      The purpose of this example is to test the use of Cheap Shadow mode under Kamui2.   */
/*                                                                                          */
/* History:                                                                                 */
...
int main (void)
{
    ...
    KMSTRIPHEAD         StripHead_17_First;     /* Params for start of cheap shadow volume. */
    KMSTRIPHEAD         StripHead_17_Normal;    /* Render Params for cheap shadow volume. */
    KMSTRIPHEAD         StripHead_17_Last;      /* Params for end of cheap shadow volume. */
    KMVERTEX_01         CurrentStrip_01[16];    /* Strip of opaque color triangles. */
    KMVERTEX_05         CurrentStrip_05[16];    /* Strip of textured triangles. */
    KMVERTEX_17         MV[12];                 /* Modifier volume (simple cube). */
    ...
    debug_write (SNASM_STDOUT, "CheapTest: Demonstrates use of Cheap Shadow Mode.", 49);
    ...
    kmsc.Pass[0].fBufferSize[1] = 50.0f;        /* Opaque modifier. */
    ...
    /* Adjust opaque polygons to be modified by shadow volume. */
    kmChangeStripShadowMode (&StripHead_05_Opaque, KM_CHEAPSHADOW_POLYGON);
    ...
    /* Reset default render context to opaque. */
    DefaultContext.ImageControl[KM_IMAGE_PARAM1].bUseAlpha = KM_FALSE;
    DefaultContext.ImageControl[KM_IMAGE_PARAM1].nTextureShadingMode = KM_MODULATE;

    /* Set cheap shadow mode intensity level (128 = 50%). */
    kmSetCheapShadowMode(128);

    /* Adjust the default render context for cheap shadow volume. */
    DefaultContext.StripControl.nListType                   = KM_OPAQUE_MODIFIER;
    DefaultContext.ObjectControl.dwModifierInstruction      = KM_MODIFIER_NORMAL_POLY;

    /* Generate a strip head arrays for polygons to form cheap shadow volume. */
    memset (&StripHead_17_First, 0, sizeof(StripHead_17_First));
    kmGenerateStripHead17 (&StripHead_17_First, &DefaultContext);
    kmChangeStripModifierInstruction (&StripHead_17_First, KM_MODIFIER_INCLUDE_FIRST_POLY);

    memset (&StripHead_17_Normal, 0, sizeof(StripHead_17_Normal));
    kmGenerateStripHead17 (&StripHead_17_Normal, &DefaultContext);
    kmChangeStripModifierInstruction (&StripHead_17_Normal, KM_MODIFIER_NORMAL_POLY);

    memset (&StripHead_17_Last, 0, sizeof(StripHead_17_Last));
    kmGenerateStripHead17 (&StripHead_17_Last, &DefaultContext);
    kmChangeStripModifierInstruction (&StripHead_17_Last, KM_MODIFIER_INCLUDE_LAST_POLY);
    ...
    while (TRUE)
	{
        ...
        /* Set up cheap shadow modifier volume (simple cube). */

        /* Front side. */
        MV[0].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[0].fAX    =     fx;      MV[0].fBX    =     fx;      MV[0].fCX    = fx + 140.0f;
        MV[0].fAY    = 290.0f;      MV[0].fBY    = 170.0f;      MV[0].fCY    = 310.0f;
        MV[0].uA.fAZ =  12.0f;      MV[0].uB.fBZ =  12.0f;      MV[0].uC.fCZ =  12.0f;

        MV[1].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[1].fAX    = fx + 140.0f; MV[1].fBX    =     fx;      MV[1].fCX    = fx + 140.0f;
        MV[1].fAY    = 310.0f;      MV[1].fBY    = 170.0f;      MV[1].fCY    = 170.0f;
        MV[1].uA.fAZ =  12.0f;      MV[1].uB.fBZ =  12.0f;      MV[1].uC.fCZ =  12.0f;

        /* Back side. */
        MV[2].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[2].fAX    = 420.0f;      MV[2].fBX    = 420.0f;      MV[2].fCX    = 220.0f;
        MV[2].fAY    = 340.0f;      MV[2].fBY    = 140.0f;      MV[2].fCY    = 340.0f;
        MV[2].uA.fAZ =  10.0f;      MV[2].uB.fBZ =  10.0f;      MV[2].uC.fCZ =  10.0f;

        MV[3].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[3].fAX    = 220.0f;      MV[3].fBX    = 420.0f;      MV[3].fCX    = 220.0f;
        MV[3].fAY    = 340.0f;      MV[3].fBY    = 140.0f;      MV[3].fCY    = 140.0f;
        MV[3].uA.fAZ =  10.0f;      MV[3].uB.fBZ =  10.0f;      MV[3].uC.fCZ =  10.0f;

        /* Left side. */
        MV[4].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[4].fAX    = 220.0f;      MV[4].fBX    = 220.0f;      MV[4].fCX    =     fx;
        MV[4].fAY    = 340.0f;      MV[4].fBY    = 140.0f;      MV[4].fCY    = 310.0f;
        MV[4].uA.fAZ =  10.0f;      MV[4].uB.fBZ =  10.0f;      MV[4].uC.fCZ =  12.0f;

        MV[5].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[5].fAX    =     fx;      MV[5].fBX    = 220.0f;      MV[5].fCX    =     fx;
        MV[5].fAY    = 310.0f;      MV[5].fBY    = 140.0f;      MV[5].fCY    = 170.0f;
        MV[5].uA.fAZ =  12.0f;      MV[5].uB.fBZ =  10.0f;      MV[5].uC.fCZ =  12.0f;

        /* Right side. */
        MV[6].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[6].fAX    = fx + 140.0f; MV[6].fBX    = fx + 140.0f; MV[6].fCX    = 420.0f;
        MV[6].fAY    = 310.0f;      MV[6].fBY    = 170.0f;      MV[6].fCY    = 340.0f;
        MV[6].uA.fAZ =  12.0f;      MV[6].uB.fBZ =  12.0f;      MV[6].uC.fCZ =  10.0f;

        MV[7].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[7].fAX    = 420.0f;      MV[7].fBX    = fx + 140.0f; MV[7].fCX    = 420.0f;
        MV[7].fAY    = 340.0f;      MV[7].fBY    = 170.0f;      MV[7].fCY    = 140.0f;
        MV[7].uA.fAZ =  10.0f;      MV[7].uB.fBZ =  12.0f;      MV[7].uC.fCZ =  10.0f;

        /* Top. */
        MV[8].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[8].fAX    =     fx;      MV[8].fBX    = 220.0f;      MV[8].fCX    = fx + 140.0f;
        MV[8].fAY    = 170.0f;      MV[8].fBY    = 140.0f;      MV[8].fCY    = 170.0f;
        MV[8].uA.fAZ =  12.0f;      MV[8].uB.fBZ =  10.0f;      MV[8].uC.fCZ =  12.0f;

        MV[9].ParamControlWord  = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[9].fAX    = fx + 140.0f; MV[9].fBX    = 220.0f;      MV[9].fCX    = 420.0f;
        MV[9].fAY    = 170.0f;      MV[9].fBY    = 140.0f;      MV[9].fCY    = 140.0f;
        MV[9].uA.fAZ =  12.0f;      MV[9].uB.fBZ =  10.0f;      MV[9].uC.fCZ =  10.0f;

        /* Bottom. */
        MV[10].ParamControlWord = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[10].fAX    = 220.0f;     MV[10].fBX    =     fx;     MV[10].fCX    = 420.0f;
        MV[10].fAY    = 340.0f;     MV[10].fBY    = 310.0f;     MV[10].fCY    = 340.0f;
        MV[10].uA.fAZ =  10.0f;     MV[10].uB.fBZ =  12.0f;     MV[10].uC.fCZ =  10.0f;

        MV[11].ParamControlWord = KM_VERTEXPARAM_ENDOFSTRIP;
        MV[11].fAX    = 420.0f;     MV[11].fBX    =     fx;     MV[11].fCX    = fx + 140.0f;
        MV[11].fAY    = 340.0f;     MV[11].fBY    = 310.0f;     MV[11].fCY    = 310.0f;
        MV[11].uA.fAZ =  10.0f;     MV[11].uB.fBZ =  12.0f;     MV[11].uC.fCZ =  12.0f;

        /* Send cheap shadow modifier volume to render system. */
        kmStartStrip (&VertexBufferDesc, &StripHead_17_First);
        kmSetVertex (&VertexBufferDesc, &MV[0], KM_VERTEXTYPE_17, sizeof (KMVERTEX_17));

        kmStartStrip (&VertexBufferDesc, &StripHead_17_Normal);
        for (i=1; i<=10; i++)
            kmSetVertex (&VertexBufferDesc, &MV[i], KM_VERTEXTYPE_17, sizeof (KMVERTEX_17));

        kmStartStrip (&VertexBufferDesc, &StripHead_17_Last);
        kmSetVertex (&VertexBufferDesc, &MV[11], KM_VERTEXTYPE_17, sizeof (KMVERTEX_17));
