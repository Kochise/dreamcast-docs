/******************************************************************************
 ...
 Created  : September 1997 by VideoLogic
...
extern  KMSTRIPCONTEXT			StripContext01;
extern  KMSTRIPHEAD				StripHead01;
extern  KMSTRIPCONTEXT			StripContext03;
extern  KMSTRIPHEAD				StripHead03;
extern 	KMSYSTEMCONFIGSTRUCT	SystemConfig;

KMSTRIPCONTEXT		StripContextBG03;
KMSTRIPHEAD			StripHeadBG03;

KMVERTEX_01			BV01[4];					/* for NO-TEXTURE-GOURAUD */
KMVERTEX_03			BV03[4];					/* for TEXTURE-GOURAUD */
 ...
 *  Description     :                                                   *
 *----------------------------------------------------------------------*/
void InitBackground2 (PKMSURFACEDESC pTextureID)
{
	float		Vpos;

	memset( &StripContextBG03, 0, sizeof(StripContextBG03));
	memset( &StripHeadBG03,	 0, sizeof(StripHeadBG03));
	InitStripContext03		( &StripContext03,&StripHeadBG03,pTextureID );
	kmChangeStripFilterMode	( &StripHeadBG03,KM_IMAGE_PARAM1,KM_BILINEAR );

	/*----------------------------------------------------------------------------------- */
	/* set VERTEX DATA */
	Vpos = (float)sin(Frame/400.0f);

	/* Vertex #3 */
	BV03[3].ParamControlWord	= KM_VERTEXPARAM_NORMAL;
	BV03[3].fX					=   0.0f;
	BV03[3].fY					= 480.0f;
	BV03[3].u.fZ				=   0.0005f;
	BV03[3].fU					=   0.0f + BILINEAR_CORRECTION;
	BV03[3].fV					=   0.0f + BILINEAR_CORRECTION + Vpos;
	BV03[3].uBaseRGB.dwPacked	= RGBAColour (255, 255, 255, 255);
	BV03[3].uOffsetRGB.dwPacked	= RGBAColour (255, 255, 255, 255);
 ...
 *  Description     :                                                   * 
 *----------------------------------------------------------------------*/
void SetBackground2 (PKMVERTEXBUFFDESC pVertexBufferDesc)
{
	float Vpos;
...
#if defined(_VERTEXMACRO_L4_)
	kmxxGetCurrentPtr(&VertexBufferDesc);
	kmxxStartStrip(&VertexBufferDesc,&StripHeadBG03);
			kmxxSetVertex_3(	BV03[3].ParamControlWord,
								BV03[3].fX,
								BV03[3].fY,
								BV03[3].u.fZ,
								BV03[3].fU,
								BV03[3].fV,
								BV03[3].uBaseRGB.dwPacked,
								BV03[3].uOffsetRGB.dwPacked
							);
			kmxxSetVertex_3(	BV03[0].ParamControlWord,
								BV03[0].fX,
								BV03[0].fY,
								BV03[0].u.fZ,
								BV03[0].fU,
								BV03[0].fV,
								BV03[0].uBaseRGB.dwPacked,
								BV03[0].uOffsetRGB.dwPacked
							);
			kmxxSetVertex_3(	BV03[2].ParamControlWord,
								BV03[2].fX,
								BV03[2].fY,
								BV03[2].u.fZ,
								BV03[2].fU,
								BV03[2].fV,
								BV03[2].uBaseRGB.dwPacked,
								BV03[2].uOffsetRGB.dwPacked
							);
			kmxxSetVertex_3(	BV03[1].ParamControlWord,
								BV03[1].fX,
								BV03[1].fY,
								BV03[1].u.fZ,
								BV03[1].fU,
								BV03[1].fV,
								BV03[1].uBaseRGB.dwPacked,
								BV03[1].uOffsetRGB.dwPacked
							);
	kmxxReleaseCurrentPtr(&VertexBufferDesc);
#else
	kmStartStrip(&VertexBufferDesc,&StripHeadBG03);

	status = kmSetVertex(&VertexBufferDesc,&BV03[3],KM_VERTEXTYPE_03,sizeof(KMVERTEX_03) );
	Check_Status(status, "kmSetVertex");

	status = kmSetVertex(&VertexBufferDesc,&BV03[0],KM_VERTEXTYPE_03,sizeof(KMVERTEX_03) );
	Check_Status(status, "kmSetVertex");

	status = kmSetVertex(&VertexBufferDesc,&BV03[2],KM_VERTEXTYPE_03,sizeof(KMVERTEX_03) );
	Check_Status(status, "kmSetVertex");

	status = kmSetVertex(&VertexBufferDesc,&BV03[1],KM_VERTEXTYPE_03,sizeof(KMVERTEX_03) );
	Check_Status(status, "kmSetVertex");
#endif
