...
/*	 Modified  : 04, February, 1999 by Hiroshi Nakayama for KAMUI2	*/
...
float CamFocus = 600.0f;
    ...
    for (i=0; i<CMesh[Object].NumVertex; i++)
	{
		...
		(PVRVert[Object]+i)->fX = f1OverZ * fX + 320;
		(PVRVert[Object]+i)->fY = -f1OverZ * fY + 240;
	    (PVRVert[Object]+i)->fInvW = f1OverZ;

		if (Effect == FX_OPTIC)
		{
			(PVRVert[Object]+i)->fUOverW = 0.5f + (fX + fZ)/210.0f;
			(PVRVert[Object]+i)->fVOverW = 0.5f + (fY + fZ)/300.0f;
		}
...
/*  Outputs         :                                                   */
/*  Returns         :                                                   */
/*  Globals Used    : OVRVertex, CMesh                                  */
/*  Description     : Copy UV values from CMesh to PVRVert              */
/*----------------------------------------------------------------------*/
...
float LightLocal[3], f1OverLength, fN[3];
	...
	for (i=0; i<CMesh[Object].NumVertex; i++)
	{
        ...
        Spec =(Col>240) ? (Col-240)*16 : 0;

		/* Setting vertex colour */
		/* Specular RGB is Highlight Color, Specular A is Vertexfog */
		(PVRVert[Object]+i)->u32Specular  = RGBAColour(Spec,Spec,Spec, 255);
	    if (Color==C_GREEN)
		{
			(PVRVert[Object]+i)->u32Colour = RGBAColour(0, Col, 0, 150);
		}
		else
		{
			(PVRVert[Object]+i)->u32Colour = RGBAColour(Col, 0, 0, 150);
		}
	}
				  ...
				  PKMSURFACEDESC pTexture)
{
	...
	SetContext (pStripHead, CInfo, pTexture);
}
