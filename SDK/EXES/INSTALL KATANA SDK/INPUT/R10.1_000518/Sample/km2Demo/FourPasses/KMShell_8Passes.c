/*****************************************************************************
  ...
  Note:  This version supports 8 passes.
...
KMSTRIPHEAD 			DCLogoStripHead; 
int						DCLogoColor = 0xFFFF3C28;
	...
	while (1)
	{
		 ...
		 */
		 for (i=0; i<nTotalPasses; i++)
		 {
			...
			/* The Dreamcast Logo (last pass)*/
			if (i==nTotalPasses-1) 	KMDisplayDCLogo ();
		
			/*
			 * Finish the pass.  
		 	 */
			if(i<nTotalPasses-1) kmContinuePass(&VertexBufferDesc);
		}
 ...
 *  Description     : Init Kamui Video and Shinobi.                                
 *----------------------------------------------------------------------*/
...
{
int i;
KMDWORD nNumPasses, dwPassInfo[8];
KMDWORD	ErrorCode;

	nTotalPasses = nNumPasses = 8;
	...
	/* Counting the passes (the first disabled breaks) */
	for (i=0; i<8; i++) 
	{
		if(dwPassInfo[i] != PASS_AUTOSORT && dwPassInfo[i] != PASS_PRESORT)
		{
		 	nTotalPasses = nNumPasses = i; 
		 	break;
		}
	}
    ...
    pVertexBuffer = (PKMDWORD) syMallocAlign32Byte (VERTEXBUFFERSIZE * sizeof (KMDWORD));
	...
	/* Passes */
	SystemConfig.nPassDepth	= nNumPasses;
	
	SystemConfig.Pass[0].nDirectTransferList = KM_OPAQUE_POLYGON;   /* Type sent direct in 2V mode. */
	if(dwPassInfo[0] == PASS_PRESORT)
			SystemConfig.Pass[0].dwRegionArrayFlag  =  KM_PASSINFO_ENABLE_Z_CLEAR | KM_PASSINFO_PRESORT;
		else
			SystemConfig.Pass[0].dwRegionArrayFlag  =  KM_PASSINFO_ENABLE_Z_CLEAR | KM_PASSINFO_AUTOSORT;
  	
   /* These amounts in % */
	SystemConfig.Pass[0].fBufferSize[0]		=	0;	/* Opaque is flushed */
	SystemConfig.Pass[0].fBufferSize[1]		=	10;	
	SystemConfig.Pass[0].fBufferSize[2]		=   70;	
	SystemConfig.Pass[0].fBufferSize[3]		=	10;	
	SystemConfig.Pass[0].fBufferSize[4]		=	10;	

	for (i=1; i<nNumPasses; i++)
	{
		SystemConfig.Pass[i].nDirectTransferList = KM_TRANS_POLYGON;   /* Type sent direct in 2V mode. */
		
	    if(dwPassInfo[i] == PASS_PRESORT)
			SystemConfig.Pass[i].dwRegionArrayFlag  =  KM_PASSINFO_ENABLE_Z_CLEAR | KM_PASSINFO_PRESORT;
		else
			SystemConfig.Pass[i].dwRegionArrayFlag  =  KM_PASSINFO_ENABLE_Z_CLEAR | KM_PASSINFO_AUTOSORT;
	
		/* These amounts in % */
		SystemConfig.Pass[i].fBufferSize[0]		=	0;	
		SystemConfig.Pass[i].fBufferSize[1]		=	0;	
		SystemConfig.Pass[i].fBufferSize[2]		=   0; /* Translucent is flushed */	
		SystemConfig.Pass[i].fBufferSize[3]		=	0;	
		SystemConfig.Pass[i].fBufferSize[4]		=	0;	
	}
