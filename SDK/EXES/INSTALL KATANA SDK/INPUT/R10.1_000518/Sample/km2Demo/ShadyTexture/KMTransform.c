										 ...
										 const float D, const float F, const float fViewAngle)
{
	...
	fHalfWidth = (float)nViewportWidth/2.0f;
	fHalfHeight = (float)nViewportHeight/2.0f;
	
	Tx = Vx / fHalfWidth;
	Ty = Vy / fHalfHeight;
	
	Eq1[0] = TransformationMatrix._11 - (TransformationMatrix._14 * Tx);
	Eq1[1] = TransformationMatrix._31 - (TransformationMatrix._34 * Tx);
	Eq1[2] = TransformationMatrix._41 + fHalfWidth  + Dy * (TransformationMatrix._21 - TransformationMatrix._24 * Tx) - TransformationMatrix._44 * Tx;
	
	Eq2[0] = TransformationMatrix._12 - (TransformationMatrix._14 * Ty);
	Eq2[1] = TransformationMatrix._32 - (TransformationMatrix._34 * Ty);
	Eq2[2] = TransformationMatrix._42 + fHalfHeight + Dy * (TransformationMatrix._22 - TransformationMatrix._24 * Ty) - TransformationMatrix._44 * Ty;
	
	Result[0] = ((Eq2[1]*Eq1[2])/Eq1[1] - Eq2[2]) / (Eq2[0] - (Eq2[1] * Eq1[0])/Eq1[1]);
	Result[1] =  -(Eq2[2] + Eq2[0]*Result[0]) / Eq2[1];
