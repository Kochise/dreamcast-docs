...
KMDWORD 		bIsTrans = FALSE;
	...
	switch(Glob_CurrentMipmappingMethod)
	{
		...
		case MIPMAP_FOURIER2:
			CreateFourierMipmapLevels(pSource, pTransSource, nSourceSize, p32BitsData, 2);
			break;
			
		case MIPMAP_FOURIER3:
			CreateFourierMipmapLevels(pSource, pTransSource, nSourceSize, p32BitsData, 6);
			break;	
	}
						...
						unsigned char *pBm, unsigned char *pBmTrans, int nSize)
{
	...
	**************************/
	if (pBmTrans)
	{
		...
		{
			lpByte=((unsigned char *)pBm+(i-1)*dwTextureSize*3+dwTextureSize*3-1);
			lpByteTrans=((unsigned char *)pBmTrans+(i-1)*dwTextureSize*3+dwTextureSize*3-1);
