...
/*===============================================================*/
/* 各国言語のキーテーブル                                        */
/* Key tables for each language                                  */
/*===============================================================*/
extern const char KeyTbl_NormalUs[];
extern const char KeyTbl_ShiftUs[];
extern const char KeyTbl_NormalUk[];
extern const char KeyTbl_ShiftUk[];

static const char* KeyTblUs[] = {
	KeyTbl_NormalUs, KeyTbl_ShiftUs,
};

static const char* KeyTblUk[] = {
	KeyTbl_NormalUk, KeyTbl_ShiftUk,
};
...
Sint32 njUserMain(void)
{
	...
	njPrintC(NJM_LOCATION(2, 2), "KEYBOARD SAMPLE #3");
	njPrintC(NJM_LOCATION(2, 3), "      -MULTI LANGUAGE-");
	njPrintH(NJM_LOCATION(30, 2), gFrameCount++, 8);

	/* キーボード言語の取得とキーテーブルの設定 */
	kdata = KbdGetRawData(hKbd);
	switch (kdata->info->lang) {
		case PDD_KBDLANG_US:
			KbdSetKeyTable(hKbd, KeyTblUs);
			break;
		case PDD_KBDLANG_UK:
			KbdSetKeyTable(hKbd, KeyTblUk);
			break;
		case PDD_KBDLANG_JP:
		default:
			KbdSetKeyTable(hKbd, NULL);
			break;
	}
