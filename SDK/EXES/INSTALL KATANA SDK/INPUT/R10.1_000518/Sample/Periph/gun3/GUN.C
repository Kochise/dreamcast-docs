 ...
 *  Module  : Gun Functions (Continuous shot/Calibration sample)
 *  File    : gun.c
 *  Date    : 1999-05-31
 *  Version : 1.03
 ********************************************************************/
...
static Sint32 GunMode;      /* どのポートがガンモードに設定されているか    */
...
static const GUNADJ ScreenAdjTblX[] = {
	/* 最小  中心  最大 */
	/* min   cen   max  */
	{ -320,    0,   320},         /* VGA                                   */
...
static const GUNADJ ScreenAdjTblY[] = {
	/* 最小  中心  最大 */
	/* min   cen   max  */
	{  -240,   0,   240},         /* VGA                                   */
...
/* Variables to memorize a last raw coordinate of a gun. */
...
static const GUNADJ* CalibX[4];
static const GUNADJ* CalibY[4];
static const GUNADJ* ScreenX[4];
static const GUNADJ* ScreenY[4];


/*-------------------------------------------------------------------------*/
static Sint32 adjust(Sint32 rawval, const GUNADJ *raw,
			const GUNADJ *adj, Sint32 min, Sint32 max, Sint32 *onscflg);
static Sint32 adjust_sub(Sint32 val, const GUNADJ *raw, const GUNADJ *adj);
...
void GunInit(Sint32 tvmode, Sint32 gunmode)
{
	...
	/* 今のところ仮 */
	LastGunPosX = ((*(volatile Uint32*)0xa05f80c4) & 0x3ff);
	LastGunPosY = ((*(volatile Uint32*)0xa05f80c4 >> 16) & 0x3ff);

	/* デフォルトのキャリブレーションテーブルを設定する */
	/* Set default calibration table. */
	for (i = 0; i < 4; i++) {
		CalibX[i] = &CalibTblX[tvmode];
		CalibY[i] = &CalibTblY[tvmode];
		ScreenX[i] = &ScreenAdjTblX[tvmode];
		ScreenY[i] = &ScreenAdjTblY[tvmode];
	}
}
...
void GunMain(void)
{
	...
	if (gunport >= 0) {
		...
		if ((LastGunPosX == gunx) && (LastGunPosY == guny)) {
			...
			gw->rawX = gunx;
			gw->rawY = guny;
			gw->x = adjust(gw->rawX, CalibX[idx], ScreenX[idx],
														-320, 319, &flag1);
			gw->y = adjust(gw->rawY, CalibY[idx], ScreenY[idx],
														-240, 239, &flag2);
...
/*===============================================================*/
/*      ガンのキャリブレーションパラメータを設定する             */
/*      Set up calibration parameter.                            */
/*===============================================================*/
...
static Sint32 adjust(Sint32 rawval, const GUNADJ *raw, const GUNADJ *adj,
