...
#define GCX_SIZE_LARGE_INFO 256 //30720 // modification for Rage, was 256
#define	GCX_SIZE_USER_DATA  30720	// addition
#define GCX_SIZE_RESERVED	(0x8000-0x7A80)  // (0x2000-0x380)
...
typedef struct GINSU_COMMAND_XFER
{
	...
	char	gcx_UserData	[GCX_SIZE_USER_DATA];	/* 200 [in/out] user program data space					*/
    char    gcx_CornerIcon  [GCX_SIZE_SMALL_INFO];  /* 7A00 [in/out] pathname of corner icon graphic         */
	char	gcx_PathDefault	[GCX_SIZE_SMALL_INFO];	/* 7A40 [in/out] working directory for default program   */
	char	_reserved		[GCX_SIZE_RESERVED];	/* 7A80          reserved                                */
}   GINSU_COMMAND_XFER_TYPE;						/*8000 END												*/
