...
' Pour les besoins de la démonstration, il n'y a pas de recherche d'une liste de composants à initialiser, 
' il crée une liste initiale avec le composant du projet Interface, comme seul membre de la liste. 
' L'extension de cette implémentation pour que votre environnement supporte une liste d'objets personnalisée
' ne devrait pas poser de problème. 
' Le schéma d'allocation peut aussi être plus sophistiqué pour refléter les besoins 
' spécifiques de gestion de votre environnement.
'
...
' Conventions d'implémentation de liste chaînée: 
  ...
  While Not bDone
    ...
    Else
      ...
      Else
        ...
        ' Parcours la chaîne d'initialisation et 
...
Sub InitPoolSvrList()
  ' Définie une liste initiale d'entrée de composants que le gestionnaire de regroupement 
  ' peut utiliser pour la démonstration.
  Const strINTERFACE_PROG_ID = "InterfaceProj.ServerInterface"
  Const strSIMPLE_PROG_ID = "SimpleProj.SimpleTimeClass"
...
Sub NodeRemove(rnNodeType As Integer, nRemoveNode As Integer)
 ' Extraire un nœud suivant d'un frère.
  If rnNodeType = gnINFO_TYPE Then
