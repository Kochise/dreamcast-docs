...
/***************************************************************************************
	student row structure for output operations (student_admin.student)
***************************************************************************************/
struct {
	char ssn[10];				// student_admin.student.ssn
	char lname[21];				// student_admin.student.lname
	char fname[13];				// student_admin.student.fname
	char name[33];				// concatenated fname and lname (including seperator space)
	char gender[8];				// student_admin.student.gender (using case function to define female or male)
	char major[5];				// student_admin.student.major
	char birth_date[31];		// student_admin.student.birth_date (allowing for format conversions)
	float tuition_paid;			// student_admin.student.tuition_paid
	float tuition_total;		// student_admin.student.tuition_total
	char start_date[31];		// student_admin.student.start_date (allowing for format conversions)
	char grad_date[31];			// student_admin.student.grad_date (allowing for format conversions)
	float loan_amount;			// student_admin.student.loan_amount
	char degree_program[15];	// student_admin.student.degree_program (using case function to redefine degree program)
	float gpa;					// calculated using case function (using student_admin.grade)
...
} studentrec;					// typedef structure


/***************************************************************************************
	grade row structure for output operations (student_admin.grade)
***************************************************************************************/
struct {
	char ssn[10];				// student_admin.grade.ssn
	char ccode[5];				// student_admin.grade.ccode
	char grade[3];				// student_admin.grade.grade
	SQLINTEGER cbssn;			// output length for ssn
...
/***************************************************************************************
	dept row structure for output operations (dept_admin.dept)
***************************************************************************************/
struct {
	char dept[5];				// dept_admin.dept
	char dname[31];				// dept_admin.dname
...
/***************************************************************************************
	class row structure for output operations (dept_admin.class)
***************************************************************************************/
struct {
	char ccode[5];				// dept_admin.grade.ccode
	char cname[31];				// dept_admin.grade.cname
	char dept[5];				// dept_admin.grade.dept
	char room[7];				// dept_admin.grade.room
	char prereq[5];				// dept_admin.grade.prereq
	SQLINTEGER cbccode;			// output length for ccode
...
void main(void)
{
	...
	logon();					// log on to the Oracle database
	menu_screen();				// Display the menu screen
	logoff();					// Log off of Oracle
...
void list_students(void)
{
	...
	if (SQLExecDirect(hstmt1,
		...
		FROM STUDENT WHERE SSN LIKE ? ORDER BY SSN",
		SQL_NTS) != SQL_SUCCESS)
	{
	...
	SQLBindCol(hstmt1, 3, SQL_C_CHAR, studentrec.gender, 7, &studentrec.cbgender);
	SQLBindCol(hstmt1, 4, SQL_C_CHAR, studentrec.birth_date, 31, &studentrec.cbbirth_date);
	SQLBindCol(hstmt1, 5, SQL_C_CHAR, studentrec.major, 5, &studentrec.cbmajor);
	SQLBindCol(hstmt1, 6, SQL_C_CHAR, studentrec.degree_program, 15, &studentrec.cbdegree_program);
	SQLBindCol(hstmt1, 7, SQL_C_FLOAT, &studentrec.tuition_paid, 0, &studentrec.cbtuition_paid);
	SQLBindCol(hstmt1, 8, SQL_C_FLOAT, &studentrec.tuition_total, 0, &studentrec.cbtuition_total);
	SQLBindCol(hstmt1, 9, SQL_C_FLOAT, &studentrec.amount_owed, 0, &studentrec.cbamount_owed);	
	SQLBindCol(hstmt1, 10, SQL_C_FLOAT, &studentrec.loan_amount, 0, &studentrec.cbloan_amount);
	SQLBindCol(hstmt1, 11, SQL_C_CHAR, studentrec.start_date, 31, &studentrec.cbstart_date);
	SQLBindCol(hstmt1, 12, SQL_C_CHAR, studentrec.grad_date, 31, &studentrec.cbgrad_date);
	SQLBindCol(hstmt1, 13, SQL_C_FLOAT, &studentrec.age_at_graduation, 0, &studentrec.cbage_at_graduation);
	...
	while (TRUE)
	{
		...
		if ((rc1 == SQL_SUCCESS) || (rc1 == SQL_SUCCESS_WITH_INFO))
		{
			...
			printf("\n\tGender: %-6s\tBirth Date: %s \n\tMajor: %-4s  Degree Program: %s",
					studentrec.gender, studentrec.birth_date, studentrec.major,
					studentrec.degree_program);
			printf("\n\tTuition Amount: %.2f \tTuition Paid: %.2f",
						studentrec.tuition_total, studentrec.tuition_paid);
			printf("\n\tTuition Owed: %.2f \tLoan Amount: %.2f",
						studentrec.amount_owed, studentrec.loan_amount);
			printf("\n\tDate Started at School: %s", studentrec.start_date);
			printf("\n\tExpected Graduation Date: %s", studentrec.grad_date);
			printf("\n\tExpected Age at Graduation: %.0f\n\n", studentrec.age_at_graduation);
...
void list_enrollments(void)
{
	...
	if (SQLExecDirect(hstmt1,
			...
			AND S.SSN LIKE ? ORDER BY S.SSN, C.CCODE", SQL_NTS) != SQL_SUCCESS)
...
void list_classes(void)
{
	...
	// execute SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT NVL(CCODE,'****') AS CCODE, \
			NVL(CNAME,'****') AS CNAME, \
			D.DEPT, \
			NVL(ROOM,'****') AS ROOM, \
...
void list_departments(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT DEPT, DNAME FROM DEPT \
...
void list_gpa(void)
{
	...
	// execute the SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT S.SSN, S.FNAME||' '||LNAME AS NAME, NVL(GPA,0) AS GPA \
						FROM STUDENT_GPA G, STUDENT S \
...
void list_student_by_class(void)
{
	...
	// execute the SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT NVL(C.CCODE,'****') AS CCODE, NVL(CNAME,'****') AS CNAME, \
							C.DEPT, NVL(ROOM,'****') AS ROOM, NVL(PREREQ,'****') AS PREREQ, \
							NVL(D.DNAME,'****') AS DNAME, NVL(G.SSN,'*********') AS SSN, \
							NVL(G.GRADE,'**') AS GRADE \
							FROM DEPT D, CLASS C, GRADE G \
...
void reluctant_students(void)
{
	...
	SQLBindParameter(hstmt1, 2, SQL_PARAM_OUTPUT, SQL_C_CHAR,
		SQL_VARCHAR, 80, 0, reluctant_student, 81, &cbreluctant_student);
	...
	while (TRUE)
	{
		...
		if (SQLExecDirect(hstmt1,
				(SQLCHAR *)"{? = call STUDENT_ADMIN.P1.SHOW_RELUCTANT_STUDENTS(?)}",
								SQL_NTS) != SQL_SUCCESS)
		{
		...
		if (rowcount == 0)
		{
			// if the rowcount = 0 (flag used to indicate all rows returned), set to old value
...
void enroll_in_entry_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
		(SQLCHAR *)"INSERT INTO GRADE (SSN, CCODE, GRADE) \
		SELECT ?, CCODE, NULL FROM CLASS \
		WHERE PREREQ IS NULL AND DEPT = ( \
			SELECT MAJOR FROM STUDENT WHERE SSN = ?)", SQL_NTS) != SQL_SUCCESS)
	{
...
void needed_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT SSN, FNAME||' '||LNAME AS NAME, MAJOR, \
						CCODE, CNAME, DEPT, NVL(ROOM,'******') ROOM, \
						NVL(PREREQ,'****') PREREQ \
						FROM STUDENT S1, CLASS C WHERE NOT EXISTS \
							(select 'X' from student s2, grade g \
							where s2.ssn = g.ssn and s2.ssn = s1.ssn \
							and g.ccode = c.ccode) AND DEPT = MAJOR \
							AND SSN LIKE ? ORDER BY S1.SSN, C.CCODE", SQL_NTS) != SQL_SUCCESS)
	{
...
void empty_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT C.CCODE, C.CNAME, C.DEPT, \
			NVL(C.ROOM,'******') ROOM, NVL(C.PREREQ,'****') PREREQ \
			FROM CLASS C WHERE NOT EXISTS \
				(SELECT 'X' FROM GRADE G WHERE G.CCODE = C.CCODE)", SQL_NTS) != SQL_SUCCESS)
	{
		// if procedure fails, display error message, return
		warning_dump();
		return;
...
void insert_student(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"INSERT INTO STUDENT (SSN,FNAME,LNAME,GENDER,MAJOR,BIRTH_DATE, \
						TUITION_PAID,TUITION_TOTAL,START_DATE,GRAD_DATE,LOAN_AMOUNT,DEGREE_PROGRAM) \
						VALUES (?, ?, ?, ?, ?, TO_DATE(?,'MM/DD/YYYY'), \
							TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?,'MM/DD/YYYY'), \
							TO_DATE(?,'MM/DD/YYYY'), TO_NUMBER(?), ?)", SQL_NTS) != SQL_SUCCESS)
	{
...
void delete_student(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"DELETE FROM STUDENT WHERE SSN = ?", SQL_NTS) != SQL_SUCCESS)
	{
...
void remove_from_class(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"DELETE FROM GRADE WHERE SSN = ? AND CCODE = ?",
				SQL_NTS) != SQL_SUCCESS)
	{
...
void delete_dept(void)
{
	...
	// execute the SQL delete statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{? = call DEPT_ADMIN.DELETE_DEPT(?)}",
					SQL_NTS) != SQL_SUCCESS)
	{
...
void insert_dept(void)
{
	...
	// execute the SQL insert statement	
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{? = call DEPT_ADMIN.INSERT_DEPT(?,?)}", SQL_NTS) != SQL_SUCCESS)
	{
...
void delete_class(void)
{
	...
	// execute the SQL delete statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{? = call DEPT_ADMIN.DELETE_CLASS(?)}",
					SQL_NTS) != SQL_SUCCESS)
	{
		// if the delete statement fails, display warning message, rollback transaction
		warning_dump();
...
void insert_class(void)
{
	...
	// execute the SQL insert statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{? = call DEPT_ADMIN.INSERT_CLASS(?,?,?,?,?)}", SQL_NTS) != SQL_SUCCESS)
	{
...
void logon(void)
{
	// allocate an ODBC enviroment handle for a Oracle connection
	SQLAllocEnv(&henv1);

	// obtain an ODBC connection handle
	SQLAllocConnect(henv1,&hdbc1);

	// connect to the requested Oracle database driver (student data)
...
void logoff(void)
{
	...
	SQLDisconnect(hdbc1);			/* Disconnect from Oracle */
...
void error_dump(void)
{
	...
	printf("\nA Oracle error occured, please review below:\n");
	printf("\nSQLState: %s\n", szSqlState);
	printf("\nErrorCode: %d\n", ErrorCode);
	printf("\nMessage: %s\n", szErrorMsg);
