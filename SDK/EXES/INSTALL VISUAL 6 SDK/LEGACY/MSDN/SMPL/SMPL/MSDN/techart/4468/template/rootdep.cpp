...
{
#include "ntddk.h"
// TODO: Add the other files to include here
}
...
// C++ specific code. Note that you provide the class definition for the 
// device extension
    C$$SAFE_ROOT$$DeviceExt *cExtension;
    cExtension = (C$$SAFE_ROOT$$DeviceExt *) DeviceObject->DeviceExtension;
...
$$IF(MACBSUPPLIESNAME)
usName = new(PagedPool) CUString(L"\\Device\\$$DEVICENAME$$");
if (!OK_ALLOCATED(usName));   // TODO: Error handling
$$ELSEIF(MACBCOMPUTESNAME)
// TODO: Here add the code to determine the device's name programatically
$$ENDIF
status = IoCreateDevice(
            DriverObject,
			sizeof(C$$SAFE_ROOT$$DeviceExt *),
$$IF(MACBUNNAMED)
            NULL,
$$ELSE
            &usName->m_String,
...
$$ENDIF
        ...
        deviceObject[$$INDEX$$]->DeviceExtension=new (NonPagedPool)C$$SAFE_ROOT$$DeviceExt;

$$IF(!MACBLUNNAMED)
// There will be a symbolic link object. Determine how to
// obtain it.
$$IF(MACBSUPPLIESLNAME)
   usLinkName= new (PagedPool)CUString(L"\\DosDevices\\$$LINKNAME$$");
   if (!OK_ALLOCATED(usLinkName));          // TODO: Error handling
$$ELSEIF(MACBCOMPUTESNAME)
// TODO: Compute the name for the link and assign it to usLinkName.
$$ENDIF
   status = IoCreateSymbolicLink (&usLinkName->m_String,&usName->m_String);
   if (!NT_SUCCESS(status));       // TODO: Add error handling
   delete(usLinkName);
   delete(usName);
$$ENDIF  //MACBLUNNAMED
