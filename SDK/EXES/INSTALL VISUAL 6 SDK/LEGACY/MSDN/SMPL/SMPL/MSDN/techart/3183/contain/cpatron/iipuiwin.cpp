...
//We use this to suppress reverting caption bars to normal
extern BOOL g_fSwitchingActive;
...
STDMETHODIMP CImpIOleInPlaceUIWindow::SetActiveObject(
   ...
   #ifdef MDI
    //CONTROLMOD
    //Control might send NULL string; so don't change caption
    if (NULL!=pIIPActiveObj && NULL!=pszObj)
    //End CONTROLMOD
        {
        TCHAR       szTitle[200];
        TCHAR       szFile[40];
        ...
        {
        /*
         * Revert to filename caption, but only if we're not activating
         * another object (thus preventing flashing).
         */
        if (!g_fSwitchingActive && NULL!=m_pDoc->m_pAdv)
