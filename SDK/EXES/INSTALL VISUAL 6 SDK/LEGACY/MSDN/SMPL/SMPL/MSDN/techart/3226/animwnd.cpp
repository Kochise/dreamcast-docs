...
BEGIN_MESSAGE_MAP(CAnimWnd, CSaveWnd)
...
BOOL CAnimWnd::Create(CPalette* pPal/*= NULL*/)
{
    ...
    return CSaveWnd::Create();
}

int CAnimWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CSaveWnd::OnCreate(lpCreateStruct) == -1)
		return -1;
	
    // Create the off-screen rendering buffer
    m_dsBuffer.Create(lpCreateStruct->cx,
                      lpCreateStruct->cy,
                      m_pPal);
...
void CAnimWnd::OnPaletteChanged(CWnd* pFocusWnd) 
{
...
BOOL CAnimWnd::OnQueryNewPalette() 
{
