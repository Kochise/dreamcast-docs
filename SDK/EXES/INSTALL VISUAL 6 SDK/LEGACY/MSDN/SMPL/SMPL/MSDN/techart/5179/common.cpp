...
void main(void)
{
	...
	logon();					// log on to the rdbms
	// is using SQL Server, use the student_db database
	if (strcmp(dbms_name,"Microsoft SQL Server") == 0) select_database();
	menu_screen();				// Display the menu screen
	logoff();					// Log off of the rdbms
...
void list_students(void)
{
	...
	if (SQLExecDirect(hstmt1,
		(SQLCHAR *)"SELECT SSN,	NAME, GENDER, BIRTH_DATE, \
			MAJOR, DEGREE_PROGRAM, TUITION_PAID, TUITION_TOTAL, \
			AMOUNT_OWED, LOAN_AMOUNT, START_DATE, GRAD_DATE, AGE_AT_GRADUATION \
			FROM LIST_STUDENTS WHERE SSN LIKE ? ORDER BY SSN",
		SQL_NTS) != SQL_SUCCESS)
	{
...
void list_enrollments(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT SSN, NAME, CCODE, CNAME, GRADE \
				FROM LIST_ENROLLMENTS \
				WHERE SSN LIKE ? ORDER BY SSN, CCODE", SQL_NTS) != SQL_SUCCESS)
...
void list_classes(void)
{
	...
	// execute SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT CCODE, CNAME, DEPT, ROOM, PREREQ, DNAME \
				FROM LIST_CLASSES \
				WHERE UPPER(DEPT) LIKE UPPER(?) ORDER BY DEPT, CCODE",
				SQL_NTS) != SQL_SUCCESS)
	{
...
void list_gpa(void)
{
	...
	// execute the SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT SSN, NAME, GPA FROM LIST_GPA \
						WHERE SSN LIKE ? ORDER BY SSN",
				SQL_NTS) != SQL_SUCCESS)
	{
		// if the SQL statement fails, display error message and return
		warning_dump();
...
void list_student_by_class(void)
{
	...
	// execute the SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT CCODE, CNAME, DEPT, ROOM, PREREQ, DNAME, SSN, GRADE \
							FROM LIST_STUDENT_BY_CLASS \
							WHERE UPPER(CCODE) LIKE UPPER(?) ORDER BY DEPT, CCODE, SSN",
								SQL_NTS) != SQL_SUCCESS)
	{
		// if SQL statement fails, display message and return
		warning_dump();
...
void reluctant_students(void)
{
	...
	// execute the SQL select statement
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT RELUCTANT_STUDENT FROM RELUCTANT_STUDENTS ORDER BY SSN",
							SQL_NTS) != SQL_SUCCESS)
	{
		// if the SQL statement fails, display error message and return
		warning_dump();
...
void needed_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT SSN, NAME, MAJOR, CCODE, CNAME, DEPT, ROOM, PREREQ \
						FROM NEEDED_CLASSES WHERE SSN LIKE ? ORDER BY SSN, CCODE",
							SQL_NTS) != SQL_SUCCESS)
	{
...
void empty_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT CCODE, CNAME, DEPT, ROOM, PREREQ \
			FROM EMPTY_CLASSES", SQL_NTS) != SQL_SUCCESS)
	{
		// if procedure fails, display error message, return
		warning_dump();
		return;
...
void insert_student(void)
{
	...
	SQLBindParameter(hstmt1, 13, SQL_PARAM_INPUT, SQL_C_CHAR,
		SQL_VARCHAR, 1, 0, indegree_program, 2, &cbindegree_program);
	
	// execute the SQL insert statement	
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{? = call INSERT_STUDENT(?,?,?,?,?,?,?,?,?,?,?,?)}",
					SQL_NTS) != SQL_SUCCESS)
	{
...
void logon(void)
{
	// allocate an ODBC enviroment handle for a rdbms connection
	SQLAllocEnv(&henv1);

	// obtain an ODBC connection handle
	SQLAllocConnect(henv1,&hdbc1);

	// connect to the requested rdbms database driver (student data)
...
void logoff(void)
{
	...
	SQLDisconnect(hdbc1);			/* Disconnect from rdbms */
...
void error_dump(void)
{
	...
	printf("\nA rdbms error occured, please review below:\n");
	printf("\nSQLState: %s\n", szSqlState);
	printf("\nErrorCode: %d\n", ErrorCode);
	printf("\nMessage: %s\n", szErrorMsg);
