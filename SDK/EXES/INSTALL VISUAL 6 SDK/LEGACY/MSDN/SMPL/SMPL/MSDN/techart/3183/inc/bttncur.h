 ...
 * values to use for the current display instead of using these
 * values directly.  However, if the application has the aspect
 * ratio already then these are available for them.
 */
 ...
 * value is mutually exclusive and contains one or more grouping
 * bits.  Each group represents buttons that share some sub-state
 * in common.
 *
 ...
 * attribute group.  Up, mouse down, and disabled states are
 * identical, but only attributes can have down, down disabled,
 * and indeterminate states.
 *
 * BUTTONGROUP_BLANK is defined so an application can draw only the
 * button without an image in the up, down, mouse down, or
 * indeterminate state, that is, BUTTONGROUP_BLANK is inclusive
 * with BUTTONGROUP_DOWN and BUTTONGROUP_LIGHTFACE.
 */
...
/*
 * Specific bits to prevent conversions of specific colors to
 * system colors.  If an application uses this newer library
 * and never specified any bits, then they benefit from color
...
#define PRESERVE_ALL                    (PRESERVE_BLACK     \
                                        | PRESERVE_DKGRAY   \
                                        | PRESERVE_LTGRAY   \
                                        | PRESERVE_WHITE)
...
typedef struct tagTOOLDISPLAYDATA
    ...
    UINT        cyBar;      //Vertical size for a bar w/ buttons.
    UINT        cxButton;   //Dimensions of a button.
    UINT        cyButton;
    UINT        cxImage;    //Dimensions of bitmap image
    UINT        cyImage;
    UINT        uIDImages;  //Standard resource ID for images
...
BOOL    WINAPI UIToolButtonDraw(HDC, int, int, int, int, HBITMAP, int
            , int, int, UINT);
BOOL    WINAPI UIToolButtonDrawTDD(HDC, int, int, int, int, HBITMAP
            , int, int, int, UINT, LPTOOLDISPLAYDATA);
