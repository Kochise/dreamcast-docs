...
Abstract:
   ...
   This file contains DirectInput functionality for the PlayMultipleSounds sample.
...
InitDirectInput()
{
    ...
    g_errLast = g_pdi->EnumDevices(DIDEVTYPE_JOYSTICK, (LPDIENUMDEVICESCALLBACK)DIEnumProc, NULL, 0);
...
CheckNewDevices()
{
    ...
    if (WaitForSingleObject(g_hNewDeviceEvent, 0) != WAIT_TIMEOUT)
        g_pdi->EnumDevices(DIDEVTYPE_JOYSTICK, (LPDIENUMDEVICESCALLBACK)DIEnumProc, NULL, 0);
}
