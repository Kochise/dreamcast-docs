...
Abstract:
   ...
   This file contains DirectInput functionality for the Microphone sample.
...
BOOL CALLBACK MicrophoneEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext);
...
InitDirectInput()
{
    ...
    // Check to see if there are any Microphone devices currently attached.
    MapleEnumerateDevices(MDT_AUDIO_IN, MicrophoneEnumProc, NULL, 0);
...
CheckNewDevices()
{
    ...
    if (WaitForSingleObject(g_hNewDeviceEvent, 0) != WAIT_TIMEOUT)
    {
        ...
        // Check to see if any of the devices are Microphones
        MapleEnumerateDevices(MDT_AUDIO_IN, MicrophoneEnumProc, NULL, 0);
    }
}
...
-------------------------------------------------------------------*/
BOOL CALLBACK
MicrophoneEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext)
{
    DWORD dwPort = pmdi->dwPort;
    if (pmdi->devType == MDT_AUDIO_IN && g_rgpcont[dwPort] != NULL)
        g_rgpcont[dwPort]->MicrophoneExists(pmdi->guidDevice);
