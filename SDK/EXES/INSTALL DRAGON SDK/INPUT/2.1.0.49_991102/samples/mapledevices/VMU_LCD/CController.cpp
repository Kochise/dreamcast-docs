...
CController::Initialize(void)
{
    ...
    // Get the 'direction' pointer value.  This value contains the direction of
    // all VMU devices plugged into the controller.  For instance, on a typical
    // joypad, the VMU is inserted in an inverted fashion, while in a lightgun
    // it is inserted "right side up".  Since the direction of the VMU devices
    // in a controller don't have to be the same, this value is used in conjunction
    // with the VMU's device number (dwDevNum) to determine the direction of a
    // particular VMU device.  See the device enumeration code (in DIUtil.cpp)
    // to see how to do that.
    g_errLast = m_pDID->GetProperty(DIPROP_EXPDIRECTION, &dipropdword.diph);
    if (CheckError(TEXT("Get Port device direction")))
        return FALSE;                                              

    m_nDirection = dipropdword.dwData;
...
Function:
    ...
    CController::LCDExists

Description:

    This is called when an LCD device is discovered to be attached to
    the controller.
...
-------------------------------------------------------------------*/
void
CController::LCDExists(GUID guidLCD)
{    
    if (m_plcd == NULL)
    {
        m_plcd = new CLCD(guidLCD, this);
        if (m_plcd == NULL)
            return;
        
        if (m_plcd->Initialize() == FALSE)
        {
            CLCD *plcdTemp = m_plcd;

            m_plcd = NULL;
            delete plcdTemp;
            OutputDebugString(TEXT("Error initializing LCD\r\n"));
