...
Function:
    ...
    CController::FlashExists

Description:

    This is called when an Flash device is discovered to be attached to
    the controller.
...
Arguments:
    ...
    int         iConn     - Connection number that this device is plugged into.
Return Value:
...
-------------------------------------------------------------------*/
void
CController::FlashExists(GUID guidFlash, int iConn)
{
	// Make it 0 based ( instead of 1 )
	iConn--;

	// If we already have a VMU in that slot, verify if it is the same
	if ( m_pflash[iConn] )
	{
		if ( guidFlash == m_pflash[iConn]->m_guidFlash ) 
			return;
		else
		{
			// The VMU found in that slot is different than what we had,
			// we need to delete the previous instance
			delete m_pflash[iConn];
			m_pflash[iConn] = NULL;
		}
	}
	
	// Create a new CFlash to represent the new VMU
    m_pflash[iConn] = new CFlash(guidFlash, iConn+1);
    if (m_pflash[iConn])
    {
        if (m_pflash[iConn]->Initialize() == FALSE)
        {
            delete m_pflash[iConn];
            m_pflash[iConn] = NULL;
            OutputDebugString(TEXT("Error initializing Flash\r\n"));
        }
    }
}
...
Function:
    ...
    CController::CheckFlashes()

Description:

    This function verifies that the flash devices we think are present still are.
...
-------------------------------------------------------------------*/
void CController::CheckFlashes()
{
	if ( m_pflash[0] )
		if ( !m_pflash[0]->FlashValid() )
		{
			delete m_pflash[0];
			m_pflash[0] = NULL;
		}
