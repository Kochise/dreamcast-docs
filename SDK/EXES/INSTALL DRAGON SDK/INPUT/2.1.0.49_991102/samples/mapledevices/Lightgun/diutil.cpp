...
Abstract:
   ...
   This file contains DirectInput functionality for the Lightgun sample.
...
BOOL CALLBACK LightGunEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext);
...
InitDirectInput()
{
    ...
    // Check to see if any of the devices are lightguns
    MapleEnumerateDevices(MDT_LIGHTGUN, LightGunEnumProc, NULL, 0);
...
Description:
    ...
    It will call this once for every lightgun currently plugged in.
...
-------------------------------------------------------------------*/
BOOL CALLBACK
LightGunEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext)
{
    DWORD dwPort = pmdi->dwPort;
    if (pmdi->devType == MDT_LIGHTGUN && g_rgpcont[dwPort] != NULL)
        g_rgpcont[dwPort]->LightgunExists(pmdi->guidDevice);
