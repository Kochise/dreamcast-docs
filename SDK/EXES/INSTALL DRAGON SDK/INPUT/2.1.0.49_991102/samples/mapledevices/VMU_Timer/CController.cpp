...
Function:
    ...
    CController::TimerExists

Description:

    This is called when an Timer device is discovered to be attached to
    the controller.
...
-------------------------------------------------------------------*/
void
CController::TimerExists(GUID guidTimer)
{    
    if (m_ptimer == NULL)
    {
        m_ptimer = new CTimer(guidTimer);
        if (m_ptimer == NULL)
            return;
        
        if (m_ptimer->Initialize() == FALSE)
        {
            CTimer *ptimerTemp = m_ptimer;

            m_ptimer = NULL;
            delete ptimerTemp;
            OutputDebugString(TEXT("Error initializing Timer\r\n"));
