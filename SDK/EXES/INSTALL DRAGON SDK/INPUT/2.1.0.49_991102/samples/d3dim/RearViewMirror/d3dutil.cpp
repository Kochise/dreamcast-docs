...
Abstract:
   ...
   This file contains Direct3D functionality for the RearViewMirror sample.
...
static D3DVECTOR g_vectFrom(0.0f, -5.0f, -1000.0f);
static D3DVECTOR g_vectAt  (0.0f, -6.0f, -950.0f);
static float     g_rRoll   = 0.0f;
static float     g_rNear   = 0.001f;
static float     g_rFar    = 3000.0f;
...
NormalizeVector(LPD3DVECTOR pvect)
{
    ...
    {
        rInvMod = (float)(1.0f / (float)sqrt(vx * vx + vy * vy + vz * vz));
        pvect->x = vx * rInvMod;
        pvect->y = vy * rInvMod;
        pvect->z = vz * rInvMod;
    }
