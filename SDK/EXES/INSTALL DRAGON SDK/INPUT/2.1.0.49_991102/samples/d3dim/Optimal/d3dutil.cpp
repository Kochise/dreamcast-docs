...
AddLight(float rRed, float rGreen, float rBlue, float rX, float rY, float rZ)
{
    ...
    g_errLast = g_pd3d->CreateLight(&g_rglight[g_nLights], NULL);
    if (CheckError(TEXT("Create Light")))
        return FALSE;
    
    // Set the global light's information
    g_errLast = g_rglight[g_nLights]->SetLight(&light);
    if (CheckError(TEXT("Set Light")))
        return FALSE;

    // Add the global light to the global viewport
    g_errLast = g_pviewport->AddLight(g_rglight[g_nLights]);
...
-------------------------------------------------------------------*/
void
SetMatrixAxisRotation(LPD3DMATRIX pM, LPD3DVECTOR pvectAxis, float rAngle)
{
    D3DVECTOR vectAxis = *pvectAxis;
    ...
    pM->_11 = t * vectAxis.x * vectAxis.x + c;
    pM->_12 = t * vectAxis.x * vectAxis.y + s * vectAxis.z;
    pM->_13 = t * vectAxis.x * vectAxis.z - s * vectAxis.y;
    pM->_14 = 0.0f;

    pM->_21 = t * vectAxis.y * vectAxis.x - s * vectAxis.z;
    pM->_22 = t * vectAxis.y * vectAxis.y + c;
    pM->_23 = t * vectAxis.y * vectAxis.z + s * vectAxis.x;
    pM->_24 = 0.0f;

    pM->_31 = t * vectAxis.z * vectAxis.x + s * vectAxis.y;
    pM->_32 = t * vectAxis.z * vectAxis.y - s * vectAxis.x;
    pM->_33 = t * vectAxis.z * vectAxis.z + c;
    pM->_34 = 0.0f;
