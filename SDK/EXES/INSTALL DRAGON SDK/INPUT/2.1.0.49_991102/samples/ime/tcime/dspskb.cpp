...
BOOL SKBRender(void)
{
    ...
    for (iKey = 0 ; iKey < g_cCurrentSKBKeys ; iKey++) {
        if (g_pskCurrentSKB == g_rgskPhonetic) {
			wcTransChar = SKBConvertPhoneticKey(g_pskCurrentSKB[iKey].szKeyString[0]);
			...
			wcTransChar = SKBConvertSymbolKey(g_pskCurrentSKB[iKey].szKeyString[0]);
		} else {
        }
		...
		if (!SKBRenderKey(g_lpddsSKB, &g_pskCurrentSKB[iKey], iKey == g_uFocusSKBKey, pszTextOverride)) {
			return FALSE;
		}
    }
                  ...
                  TCHAR*              pszTextOverride)
{
    ...
    xKeyLeftEdge = psk->xGridLocation * SKB_COLWIDTH + SKB_BORDER_THICKNESS;
    yKeyTopEdge = psk->yGridLocation * SKB_ROWHEIGHT + SKB_BORDER_THICKNESS;
