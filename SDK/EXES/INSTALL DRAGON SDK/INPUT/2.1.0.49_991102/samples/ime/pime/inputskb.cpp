    ...
    )
{
    ...
    if (g_pdidevice2JoystickSKB)
    {
                ...
                switch (psk->szKeyString[0]) 
                {
                    case HIRAGANA_SWITCH:
                        ...
                        if (g_pskCurrentSKB != g_rgskKatakana) 
                        {
                            ...
                            SetIMEState(g_pkeybdstateCurrent);
                        } else {
                    ...
                    case KANJI_HALF_0:
                        if (g_pskCurrentSKB == g_rgskKatakana) 
                        {
							...
							if (g_pkeybdstateCurrent->fHalfWidth) {
								// dummy message to toggle app state for skb redraw
								SendMessage(g_hwndApp, WM_KEYDOWN, VK_DBE_DBCSCHAR, 0);
								// message for IME
								PostMessage(g_hwndApp, WM_KEYDOWN, VK_DBE_DBCSCHAR, 0);
								SKBRender();
							} else {
								// dummy message to toggle app state for skb redraw
								SendMessage(g_hwndApp, WM_KEYDOWN, VK_DBE_SBCSCHAR, 0);
								// message for IME
								PostMessage(g_hwndApp, WM_KEYDOWN, VK_DBE_SBCSCHAR, 0);
								SKBRender();
							}
                        } 
                    ...
                    case SYMBOLS_DOWN:
						ch = ' ';
                        PostKeybdMessage((HWND) -1, psk->vkCode, KeyStateDownFlag, 0, &dummy, &ch);
                        break;        
                    default:
        ...
        if (g_fAnalogJoystick || g_fConverting)
        {
