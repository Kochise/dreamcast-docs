    ...
    )
{
	...
	if (ERROR_SUCCESS != RegOpenKeyEx (HKEY_LOCAL_MACHINE, 
        ...
        OutputDebugString(TEXT("PIME: ERROR: Japanese Pocket IME not enabled.\r\n\r\n"));
        OutputDebugString(TEXT("PIME: To fix this, use the Configuration Tool to turn on the \"Pocket IME\"\r\n"));
        OutputDebugString(TEXT("PIME: and \"Japanese font\", and then rebuild the image.  \r\n\r\n"));
        OutputDebugString(TEXT("PIME: Alternatively, from the WinCE command window, type \"set IMGPIME=1\" \r\n"));
        OutputDebugString(TEXT("PIME: and \"set COUNTRY=JAPAN\", then run \"makeimg\".\r\n"));
        return FALSE;
    } else {
        RegCloseKey (hkey);
    }
    ...
    if (GetACP() != 932) {
        OutputDebugString(TEXT("PIME: ERROR: Default codepage is not Japanese codepage.\r\n\r\n"));
        OutputDebugString(TEXT("PIME: To fix this, use the Configuration Tool to turn on the \"Pocket IME\"\r\n"));
        OutputDebugString(TEXT("PIME: and \"Japanese font\", and then rebuild the image.  \r\n\r\n"));
        OutputDebugString(TEXT("PIME: Alternatively, from the WinCE command window, type \"set IMGPIME=1\" \r\n"));
        OutputDebugString(TEXT("PIME: and \"set COUNTRY=JAPAN\", then run \"makeimg\".\r\n"));
