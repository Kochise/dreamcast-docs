...
Abstract:
	...
	Demonstrates how to play a Format 1 Standard Midi File format file
    on Dreamcast, which only accepts Format 0 data.  This is accomplished
	by converting the multiple tracks from a Format 1 file into a single track.
...
WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    g_errLast = midiStreamProperty(hms, (LPBYTE)&mpt, MIDIPROP_GET | MIDIPROP_TEMPO);
    if (CheckError(TEXT("Get tempo")))
        goto done;	
    ...
    if (*lpCmdLine == NULL)
    {
        ...
        RetailOutput(TEXT("Usage:   s format1 <MIDI filename>\r\n\r\n"));

        RetailOutput(TEXT("Example: s format1 demomidi.mid\r\n"));
        RetailOutput(TEXT("Example: s format1 \\PC\\Samples\\demomidi.mid\r\n"));
        goto done;
    }
    ...
    if (!LoadSequence(&mhOutput, szFilename, &td, mpt.dwTempo))
    {
    ...
    g_errLast  = midiOutReset((HMIDIOUT)hms);
    if (CheckError(TEXT("Reset MIDI Stream Device")))
        goto done;
