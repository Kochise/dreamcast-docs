/*****************************************************************************
  ...
  Description: 2x2, 4x4 and Fourier mipmapping (see D3DTools/D3DTTextures.c)
...
void ShowStartUpMessage			(LPDIRECT3DDEVICE3 lpDev3);
...
TCHAR  *MimpTx[] = {L"No Mipmap",L"MM 2x2",L"   MM 4x4",L"Fourier"};

	if(PadControl()==FALSE) return FALSE; 

	/* First frame shows a "Waiting..." message.
	 * Textures are loaded (and mipmapped) in the second frame.
	 */
	if(nFrame==0)
	{
		ShowStartUpMessage (lpDev3);
		return TRUE;
	}
	else if (nFrame==1)
	{
		LoadTextures (lpGlobDD4, lpDev3);
	}
	...
	for (i=0; i<4; i++)
	{
		...
		D3DTMatrixRotate    (&WorldMatrix, fAng1, fAng2, 0.0f);
		D3DTMatrixTranslate (&WorldMatrix, 0.0f, 0.0f, fDist+(float)i); 

		/* Transform vertices (no lighting) */
		TransformObject(lpDev3, &WorldMatrix, &D3DObject[M_BOX], TRUE, FALSE, USE_D3DTLVERTEX);
	
		/* Re-locate the cube */
		PosX = CubPosX[i] * 400.0f/(fDist+200.0f);
		PosY = CubPosY[i] * 400.0f/(fDist+200.0f);						
		for (j=0; j<D3DObject[M_BOX].nNumberOfVertices; j++)
		{
			(D3DObject[M_BOX].pTLVertices+j)->sx += PosX;
			(D3DObject[M_BOX].pTLVertices+j)->sy += PosY;
		}
		
		if(nTexture==1) /* Translucent */
		{
		...
		/* Render the cube */
		RenderObjectTriangles (lpDev3, D3DTexture[i*4+nTexture].lpD3DTexture, &D3DObject[M_BOX], USE_D3DTLVERTEX);

		/* Draw info banners */
		LettX = (CubPosX[i]-21) * 400.0f/(fDist+200.0f) * 1.0f/2.2f + 50;
		LettY = CubPosY[i] * 400.0f/(fDist+200.0f) * 1.0f/4.0f + 50;		
		
		D3DTPrint3D (LettX, LettY, 300.0f/(fDist+200.0f), 0xFFA0A0A0, L"%s", MimpTx[i]);	
	}
 ...
 *					so you can delete anything you created previously 
 *******************************************************************************/
...
register i;
	...
	RELEASE(D3DTexture[20].lpDDSTextureSurface);
	RELEASE(D3DTexture[20].lpD3DTexture);
...
{
	/* InfoWin */
	InfoWin = D3DTPrint3DCreateDefaultWindow (1.0f, 93.0f, 45, NULL, L"Press (A/B) to change texture");

	/* Window 0 */
	TWin[0] = D3DTPrint3DCreateDefaultWindow (1.0f, 1.0f, 45, L"MIPMAPPING ", NULL);
	
	/* Window 1 */
	TWin[1] = D3DTPrint3DCreateDefaultWindow (1.0f, 1.0f, 45, L"MIPMAPPING (1 of 3) ", L"\
2x2\n\n\
...
Each texel of the current mipmap level is calculated as a 2 by 2 texel sample from the higher mipmap level.");
	...
	/* Window 2 */
	TWin[2] = D3DTPrint3DCreateDefaultWindow (1.0f, 1.0f, 45, L"MIPMAPPING (2 of 3) ", L"\
4x4\n\n\
...
Basically 16 texels from one level are used in a weighted sum to produce a single texel in the next level.");
	...
	/* Window 3 */
	TWin[3] = D3DTPrint3DCreateDefaultWindow (1.0f, 1.0f, 45, L"MIPMAPPING (3 of 3) ", L"\
FOURIER\n\n\
...
WORD pFaces[] = {0,1,2, 0,2,3};
     ...
     Plane[0].sx		= 0.0f;
	 Plane[0].sy		= 0.0f;
	 Plane[0].rhw		= 0.00001f;
	 Plane[0].sz		= 1.0f - 0.00001f;
	 ...
	 Plane[1].sy		= 0.0f;
	 Plane[1].rhw		= 0.00001f;
	 Plane[1].sz		= 1.0f - 0.00001f;
	 ...
	 Plane[2].rhw		= 0.00001f;
	 Plane[2].sz		= 1.0f - 0.00001f;
	 ...
	 Plane[3].rhw		= 0.00001f;
	 Plane[3].sz		= 1.0f - 0.00001f;
...
DDPIXELFORMAT *PixelFormat;
	...
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP1"), NULL, FALSE, D3DTAutoMipmap_None,
	...
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP2"), TEXT("TBMP_MIPMAP2"), FALSE, D3DTAutoMipmap_None,
						 PixelFormat, TRUE, &D3DTexture[1].lpDDSTextureSurface, &D3DTexture[1].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP3"), NULL, FALSE, D3DTAutoMipmap_None,
						 PixelFormat, TRUE, &D3DTexture[2].lpDDSTextureSurface, &D3DTexture[2].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP4"), NULL, FALSE, D3DTAutoMipmap_None,
						 PixelFormat, TRUE, &D3DTexture[3].lpDDSTextureSurface, &D3DTexture[3].lpD3DTexture);

	/* 2x2 Mipmapping */
	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP1"), NULL, FALSE, D3DTAutoMipmap_2by2,
						 PixelFormat, TRUE, &D3DTexture[4].lpDDSTextureSurface, &D3DTexture[4].lpD3DTexture);

	PixelFormat = D3DTTextureBestTranslucentFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP2"), TEXT("TBMP_MIPMAP2"), FALSE, D3DTAutoMipmap_2by2,
						 PixelFormat, TRUE, &D3DTexture[5].lpDDSTextureSurface, &D3DTexture[5].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP3"), NULL, FALSE, D3DTAutoMipmap_2by2,
						 PixelFormat, TRUE, &D3DTexture[6].lpDDSTextureSurface, &D3DTexture[6].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP4"), NULL, FALSE, D3DTAutoMipmap_2by2,
						 PixelFormat, TRUE, &D3DTexture[7].lpDDSTextureSurface, &D3DTexture[7].lpD3DTexture);

	/* 4x4 Mipmapping */
	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP1"), NULL, FALSE, D3DTAutoMipmap_4by4,
						 PixelFormat, TRUE, &D3DTexture[8].lpDDSTextureSurface, &D3DTexture[8].lpD3DTexture);

	PixelFormat = D3DTTextureBestTranslucentFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP2"), TEXT("TBMP_MIPMAP2"), FALSE, D3DTAutoMipmap_4by4,
						 PixelFormat, TRUE, &D3DTexture[9].lpDDSTextureSurface, &D3DTexture[9].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP3"), NULL, FALSE, D3DTAutoMipmap_4by4,
						 PixelFormat, TRUE, &D3DTexture[10].lpDDSTextureSurface, &D3DTexture[10].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP4"), NULL, FALSE, D3DTAutoMipmap_4by4,
						 PixelFormat, TRUE, &D3DTexture[11].lpDDSTextureSurface, &D3DTexture[11].lpD3DTexture);

	/* Fourier mipmapping */
	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP1"), NULL, FALSE, D3DTAutoMipmap_Fourier_LP10,
						 PixelFormat, TRUE, &D3DTexture[12].lpDDSTextureSurface, &D3DTexture[12].lpD3DTexture);

	PixelFormat = D3DTTextureBestTranslucentFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP2"), TEXT("TBMP_MIPMAP2"), FALSE, D3DTAutoMipmap_Fourier_LP10,
						 PixelFormat, TRUE, &D3DTexture[13].lpDDSTextureSurface, &D3DTexture[13].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP3"), NULL, FALSE, D3DTAutoMipmap_Fourier_LP10,
						 PixelFormat, TRUE, &D3DTexture[14].lpDDSTextureSurface, &D3DTexture[14].lpD3DTexture);

	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_MIPMAP4"), NULL, FALSE, D3DTAutoMipmap_Fourier_LP10,
						 PixelFormat, TRUE, &D3DTexture[15].lpDDSTextureSurface, &D3DTexture[15].lpD3DTexture);


	/* Background */
	PixelFormat = D3DTTextureBestOpaqueFormat(TRUE);
	D3DTTextureLoadBMP(lpDD4, lpDev3, TEXT("BMP_BACK"), NULL, FALSE, D3DTAutoMipmap_None,
						 PixelFormat, TRUE, &D3DTexture[20].lpDDSTextureSurface, &D3DTexture[20].lpD3DTexture);
...
void ShowStartUpMessage (LPDIRECT3DDEVICE3 lpDev3)
{
	...
	D3DTPrint3D (15.0f, 45.0f, 1.0f, 0xFFFFFFFF, L"Calculating Fourier mipmaps...");
