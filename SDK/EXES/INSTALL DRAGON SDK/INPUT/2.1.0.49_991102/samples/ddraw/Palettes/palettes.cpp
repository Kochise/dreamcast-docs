...
// ++++ Local Variables +++++++++++++++++++++++++++++++++++++++++++++
TCHAR     g_tszAppName[] = TEXT("Palettes");        // The App's Name
LPDIRECTDRAWSURFACE g_pddsSample;                   // The sample bitmap to display
LPDIRECTDRAWPALETTE g_pddpal[4];                    // The four palettes we rotate through.
...
UpdateFrame ()
{
    ...
    // Rotate through the palette entries once per second
    dwCurTime = GetTickCount();

    if (dwCurTime - s_dwTime >= 1000)
    {
        // Set next palette
        g_pddsSample->SetPalette(g_pddpal[s_iPal]);
...
AppInit(HINSTANCE hPrev, int nCmdShow)
{
    ...
    g_pddsSample = LoadBitmapToPalettizedDDS(TEXT("SampleBitmap"));
    if (!g_pddsSample)
        return FALSE;

    // Create the palettes that we will rotate through
    InitPalettes();
...
Description:
    ...
    Set the specified palette entry in the specified palette entry array

Arguments:

    PALETTEENTRY    *prrgppe        - Array of palette entries - we'll set one of them here

    int             nEntry          - The entry to set (0-255)

    int             nRed            - Red value

    int             nGreen          - Green value

    int             nBlue           - Blue value
...
-------------------------------------------------------------------*/
static void
SetPalEntry(PALETTEENTRY *prgppe, int nEntry, int nRed, int nGreen, int nBlue)
{
    prgppe[nEntry].peRed   = nRed;
    prgppe[nEntry].peGreen = nGreen;
    prgppe[nEntry].peBlue  = nBlue;
}
...
Description:
    ...
    Initialize the four global palettes that we will rotate through
...
InitPalettes()
{
    ...
    // First palette is the palette of the sample bitmap
    g_pddsSample->GetPalette(&g_pddpal[0]);

    // Second palette is a red gradient
    for (i = 0; i < 256; i++)
        SetPalEntry(ppe, i, i, 0, 0);
    g_pdd->CreatePalette(DDPCAPS_8BIT, ppe, &g_pddpal[1], NULL);

    // Third palette is a green gradient
    for (i = 0; i < 256; i++)
        SetPalEntry(ppe, i, 0, i, 0);
    g_pdd->CreatePalette(DDPCAPS_8BIT, ppe, &g_pddpal[2], NULL);

    // Fourth palette is a blue gradient
    for (i = 0; i < 256; i++)
        SetPalEntry(ppe, i, 0, 0, i);
    g_pdd->CreatePalette(DDPCAPS_8BIT, ppe, &g_pddpal[3], NULL);
}
