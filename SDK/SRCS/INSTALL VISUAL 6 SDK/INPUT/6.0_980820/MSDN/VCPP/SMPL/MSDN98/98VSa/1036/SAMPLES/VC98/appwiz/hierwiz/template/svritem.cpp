...
$$IF(WANTS_TEXTVIEW)
/////////////////////////////////////////////////////////////////////////////
// CSTextView drawing helpers

BOOL CServerNode::GetNodeText(CString *strBuf, int TAB)
{
	*strBuf+= m_strDescription ;
	//Insert a return
	*strBuf+= "\r\n" ;
	return TRUE;
}

int CServerNode::GetTreeText(CString * strBuf, int TAB)
{
	ASSERT(strBuf != NULL);

	// Indent if we need to.
	for(int i = 0; i< TAB; i++)
		*strBuf+= "\t" ;

	GetNodeText(strBuf, TAB);
	if (m_listChild.GetCount() == 0 || m_bHideChildren)
		return TAB;     // nothing more to draw
	...
	while (pos != NULL)
	{
		CServerNode *pChild = (CServerNode*)m_listChild.GetNext(pos);
		// draw the child node
		pChild->GetTreeText(strBuf, TAB +1);
	}
	return TAB;
}
