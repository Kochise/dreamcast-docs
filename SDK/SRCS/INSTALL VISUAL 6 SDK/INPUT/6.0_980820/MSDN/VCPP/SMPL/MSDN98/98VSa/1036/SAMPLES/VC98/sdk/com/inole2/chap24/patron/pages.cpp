...
BOOL CPages::Init(HWND hWndParent, LPRECT pRect, DWORD dwStyle
    ...
    int         cy;
    //CHAPTER24MOD
    FONTDESC    fd={sizeof(FONTDESC), OLETEXT("Arial")
    ...
    OleCreateFontIndirect(&fd, IID_IFontDisp, (void **)&m_pIFont);
    //End CHAPTER24MOD
 ...
 * CPages::ToggleUIDead
 * CPages::ToggleHatchHandles
 *
 * Purpose:
 *  Pass-through to CPage toggles.
 */
...
void CPages::ToggleUIDead(BOOL fUIDead)
    {
    if (NULL==m_pPageCur)
        return;

    m_fUIDead=fUIDead;
    m_pPageCur->ToggleUIDead(fUIDead);
    return;
    }


void CPages::ToggleHatchHandles(BOOL fHatchHandles)
    {
    if (NULL==m_pPageCur)
        return;

    m_fHatchHandles=fHatchHandles;
    m_pPageCur->ToggleHatchHandles(fHatchHandles);
    return;
    }
...
BOOL CPages::TryMnemonic(LPMSG pMsg)
    {
    if (NULL!=m_pPageCur)
        return m_pPageCur->TryMnemonic(pMsg);
