...
CCosmoDoc::~CCosmoDoc(void)
    ...
    //CHAPTER14MOD
    if (0L!=m_dwActiveFig)
        RevokeActiveObject(m_dwActiveFig, NULL);

    DeleteInterfaceImp(m_pAutoFig);
    //End CHAPTER14MOD
...
BOOL CCosmoDoc::Init(PDOCUMENTINIT pDI)
    ...
    m_pAutoFig=new CAutoFigure(this);

    if (NULL==m_pAutoFig)
        return FALSE;

    if (!m_pAutoFig->Init(TRUE))
        return FALSE;

    /*
     * As with the application object, we'll register as strong
     * here and implement IExternalConnection to control closure
     * of the document when all external connections are gone.
     * IExternalConnection comes from CAutoBase.
     */
    RegisterActiveObject((IUnknown *)m_pAutoFig
        , CLSID_CosmoFigure, ACTIVEOBJECT_STRONG, &m_dwActiveFig);
    //End CHAPTER14MOD
...
//CHAPTER14MOD
/*
 * CCosmoDoc::AutoFigure
 *
 * Purpose:
 *  Returns the 'figure' automation object pointer for this
 *  document.
 *
 * Return Value:
 *  PCAutoFigure    Pointer to the figure object
 */

PCAutoFigure CCosmoDoc::AutoFigure(void)
    {
    return m_pAutoFig;
    }
//End CHAPTER14MOD
