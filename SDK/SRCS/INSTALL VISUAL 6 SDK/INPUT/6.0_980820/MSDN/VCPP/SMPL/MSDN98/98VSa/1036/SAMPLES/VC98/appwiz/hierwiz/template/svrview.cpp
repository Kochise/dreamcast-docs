...
$$IF(WANTS_TEXTVIEW)
	ON_COMMAND(ID_VIEW_TEXT_VIEW, OnViewTextView)
$$ENDIF
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
...
void CServerView::OnImportText()
{
	...
	OpenTextFile(dlg.GetPathName());

}

void CServerView::OpenTextFile(CString Name)
{
	CFile file;
...
static int ReadString(CArchive& ar, char* pString, int nMaxLen)
{
	...
	TRY
	{
		do
		{
			...
			//Remove any extra \r
			if (ch == '\r' && nCount != 0 && pString[nCount-1] == '\r')
				nCount--;
...
$$IF(WANTS_TEXTVIEW)
void CServerView::OnViewTextView()
{
	// Show the active document in text view.
	CServerDoc* pServerDoc = GetDocument();
	CDocument * pNewDoc ;
	pNewDoc = ((CServerApp *)AfxGetApp())->pDocTxtTemplate->OpenDocumentFile(NULL);

   //Detach the view from the newly created doc and attach it to the CServerDoc document
	POSITION pos = pNewDoc->GetFirstViewPosition() ;
	CTextView * pTxtvw = ( CTextView *)pNewDoc->GetNextView(pos) ;
	pNewDoc->RemoveView(pTxtvw) ;
	pServerDoc->AddView(pTxtvw) ;
	pTxtvw->RedrawText() ; // Do an initial draw of textview

}
void CServerView::CreateTextView()
{
	OnViewTextView() ;
}
