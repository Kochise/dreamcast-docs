...
<SCRIPT LANGUAGE=javascript RUNAT=Server>
	...
	function DTCRecordset1_ondatasetcomplete() {
		if (Request("InCart") == 0) {
			fieldsArray = new Array("SessionID","ProductID","ProductName","UnitPrice","Qty");
			valuesArray = new Array(Session.SessionID,Request("ProdID"),Request("ProdName"),Request("Price"),Request("intQty"));
			DTCRecordset1.addImmediate(fieldsArray,valuesArray);		
		}
		else {
			DTCRecordset1.fields.setValue("SessionID",Session.SessionID);
			DTCRecordset1.fields.setValue("ProductID",Request("ProdID"));
			DTCRecordset1.fields.setValue("ProductName",Request("ProdName"));
			DTCRecordset1.fields.setValue("UnitPrice",Request("Price"));
			DTCRecordset1.fields.setValue("Qty",Request("intQty"));
...
function _initDTCRecordset1()
{
	...
	cmdTmp.CommandText = 'Sélectionner * dans le chariot des courses';
	rsTmp.CacheSize = 10;
	rsTmp.CursorType = 3;
