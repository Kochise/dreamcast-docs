...
BOOL CDIBPal::Create(CDIB *pDIB)
{
    ...
    pPal = (LPLOGPALETTE) malloc(sizeof(LOGPALETTE) + dwColors * sizeof(PALETTEENTRY));
    if (!pPal) {
    ...
    for (dw=0; dw<dwColors; dw++) {
        pPal->palPalEntry[dw].peRed = lpRGB[dw].rgbRed;
        pPal->palPalEntry[dw].peGreen = lpRGB[dw].rgbGreen;
        pPal->palPalEntry[dw].peBlue = lpRGB[dw].rgbBlue;
