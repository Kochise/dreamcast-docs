...
// 3:Note: OLECREATE macros not reqd. as we are just supplying an interface
// and not allowing creation of this object
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_pal.SetSysPalColors(); // make identity palette
    m_dibBkgnd.MapColorsToPalette(&m_pal);
                                 ...
                                 CRect* pRect)
{
    ...
    pIDrawing->SetPalette((HPALETTE)m_pal.m_hObject);
    ...
    if ((*ppIUnknown)->QueryInterface(IID_INotifySrc, (LPVOID*)&pINotifySrc) != S_OK) {
        ...
        ASSERT(pINotify); 
        pINotifySrc->AddUser(pINotify, *ppIUnknown);
...
void CMainFrame::DrawObject(CDC* pDC, IUnknown* pIUnknown, int x, int y)
{
    ...
    if (pIUnknown->QueryInterface(IID_IDrawing, (LPVOID*)&pIDrawing) == S_OK) {
        ...
        // Ask it to draw
        ASSERT(pIDrawing);
        if (pIDrawing->Draw(pDC->GetSafeHdc(), x, y) != S_OK) {
...
void CMainFrame::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    ...
    if (pIUnknown->QueryInterface(IID_IApplianceUI, (LPVOID*)&pIApplianceUI) == S_OK) {
        ...
        pIApplianceUI->ShowControl(this->GetSafeHwnd());
