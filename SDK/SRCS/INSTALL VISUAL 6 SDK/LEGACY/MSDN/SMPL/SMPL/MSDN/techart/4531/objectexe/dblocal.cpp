...
BOOL CDBLocalApp::InitInstance()
{
	...
	if (m_lpCmdLine[0] != '\0')
	{
		if (lstrcmpi(m_lpCmdLine, _T("/REGSERVER"))==0) {
			if (FAILED(DllRegisterServer())) {
				AfxMessageBox(_T("Unable to register the server!"));
				}
			return FALSE;
			}
		else if (lstrcmpi(m_lpCmdLine, _T("/UNREGSERVER"))==0) {
			if (FAILED(DllUnregisterServer())) {
				AfxMessageBox(_T("Unable to unregister the server!"));
				}
			return FALSE;
			}
	}
