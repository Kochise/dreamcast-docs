...
BOOL CPage::FOpen(LPSTORAGE pIStorage)
    ...
    if (FTenantGet(0, &m_pTenantCur, FALSE))
        m_pTenantCur->Select(TRUE, !m_pPG->m_fDesignMode);
    ...
    if (0==--m_cOpens)
        ...
        for (i=0; i < m_cTenants; i++)
            ...
            if (FTenantGet(i, &pTenant, FALSE))
                {
                if (NULL!=m_hWnd)
                    pTenant->Select(FALSE, !m_pPG->m_fDesignMode);
...
BOOL CPage::TenantCreate(TENANTTYPE tType, LPVOID pv
    ...
    if (NULL!=m_pTenantCur)
        m_pTenantCur->Select(FALSE, !m_pPG->m_fDesignMode);
    ...
    m_pTenantCur->RectSet(&rcl, FALSE, FALSE);
 ...
 *
 * Purpose:
 *  Loops through all the tenants and in-place deactivates
 *  or activates any objects.  When activating, also UI
