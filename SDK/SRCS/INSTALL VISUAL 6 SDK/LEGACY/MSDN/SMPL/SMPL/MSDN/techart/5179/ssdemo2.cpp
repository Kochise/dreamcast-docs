...
void list_students(void)
{
	...
	// use a server cursor for incremental fetching
	SQLSetStmtOption(hstmt1,SQL_CURSOR_TYPE,SQL_CURSOR_DYNAMIC);
	...
	// first fetch to the end to fill client-side cursor, then reposition to start
	rc1 = SQLExtendedFetch(hstmt1, SQL_FETCH_LAST, 0, NULL, NULL);
	if (rc1 == SQL_ERROR)
		{
			// if a fetch (read) fails, display error message and stop program
			error_dump();
		}
	rc1 = SQLExtendedFetch(hstmt1, SQL_FETCH_ABSOLUTE, 0, NULL, NULL);
	if (rc1 == SQL_ERROR)
		{
			// if a fetch (read) fails, display error message and stop program
			error_dump();
	...
	while (TRUE)
	{
		rc1 = SQLExtendedFetch(hstmt1, SQL_FETCH_NEXT,0,NULL, NULL);
		if (rc1 == SQL_ERROR)
		{
			// if a fetch (read) fails, display error message and stop program
			error_dump();
...
void logon(void)
{
	...
	// ask for the driver manager to create a buffered, snapshot cursor
	SQLSetConnectOption(hdbc1, SQL_ODBC_CURSORS, SQL_CUR_USE_ODBC);
	...
	// set default behavior to use a client-side snapshot cursor
	SQLSetStmtOption(NULL,SQL_CURSOR_TYPE,SQL_CURSOR_STATIC);
