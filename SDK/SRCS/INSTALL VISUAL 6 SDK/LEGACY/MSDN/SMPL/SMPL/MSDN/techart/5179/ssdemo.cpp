...
STUDENTREC studentrec;			// student output record
...
void select_database(void)
{
	...
	rc1 = SQLExecDirect(hstmt1,
			(SQLCHAR *)"USE USER_DB", SQL_NTS);
	
	// If unable to use the student_db database, return an error message
	if ( (rc1 != SQL_SUCCESS) && (rc1 != SQL_SUCCESS_WITH_INFO) )
	{
		printf("\nUnable to access the USER_DB DATABASE.");
		error_dump();
	}
	else printf("\nSuccessfully logged on to the USER_DB DATABASE.");
...
{
	#define ROWS 5   // sample row buffer size, usually would be larger
	UDWORD    crow, irow;
	...
	FROM STUDENT_ADMIN.STUDENT WHERE SSN LIKE ? ORDER BY SSN",
		SQL_NTS) != SQL_SUCCESS)
	{
	...
	SQLSetStmtOption(hstmt1, SQL_BIND_TYPE, sizeof(rstudentrec[ROWS]));
		...
		if ((rc1 == SQL_SUCCESS) || (rc1 == SQL_SUCCESS_WITH_INFO))
		{
			for (irow = 0; irow < crow; irow++) 
			{
				...
				if (rgfRowStatus[irow] != SQL_ROW_DELETED && rgfRowStatus[irow] != SQL_ROW_ERROR)
				{
					...
					printf("\nStudent Social Security Number: %s \tName: %s", rstudentrec[irow].ssn, rstudentrec[irow].name);
...
void list_departments(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"SELECT DEPT, DNAME FROM DEPT_ADMIN.DEPT \
...
void reluctant_students(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{?=call STUDENT_ADMIN.SHOW_RELUCTANT_STUDENTS}", SQL_NTS) != SQL_SUCCESS)
	{
...
void enroll_in_entry_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
		(SQLCHAR *)"INSERT INTO STUDENT_ADMIN.GRADE (SSN, CCODE, GRADE) \
		SELECT ?, CCODE, NULL FROM DEPT_ADMIN.CLASS \
		WHERE PREREQ IS NULL AND DEPT = ( \
			SELECT MAJOR FROM STUDENT_ADMIN.STUDENT WHERE SSN = ?)", SQL_NTS) != SQL_SUCCESS)
	{
...
void needed_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{call STUDENT_ADMIN.CLASSES_NEEDED_TO_GRADUATE(?)}", SQL_NTS) != SQL_SUCCESS)
	{
...
void empty_classes(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"{call DEPT_ADMIN.SHOW_EMPTY_CLASSES}", SQL_NTS) != SQL_SUCCESS)
	{
		// if procedure fails, display error message, return
		warning_dump();
		return;
...
void insert_student(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"INSERT INTO STUDENT_ADMIN.STUDENT (SSN,FNAME,LNAME,GENDER,MAJOR,BIRTH_DATE, \
...
void delete_student(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"DELETE FROM STUDENT_ADMIN.STUDENT WHERE SSN = ?", SQL_NTS) != SQL_SUCCESS)
	{
...
void remove_from_class(void)
{
	...
	if (SQLExecDirect(hstmt1,
			(SQLCHAR *)"DELETE FROM STUDENT_ADMIN.GRADE WHERE SSN = ? AND CCODE = ?",
				SQL_NTS) != SQL_SUCCESS)
	{
