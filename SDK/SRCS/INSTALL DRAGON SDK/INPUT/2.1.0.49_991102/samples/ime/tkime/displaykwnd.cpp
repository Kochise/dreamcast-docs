    ...
    )
{
    ...
    if (!DDrawFillRect (g_lpddsKWND, 0, KWND_BORDER_HEIGHT, 
        KWND_WIDTH, KWND_KEYS_HEIGHT, KWND_BKGNDCOLOR))
    {
    ...
    if (!RenderBlankKey (g_lpddsKWND, KWND_WIDTH - KWND_SCROLLBARWIDTH, 
    ...
    )
{
    ...
    xKeyLeftEdge = xKWNDLeftEdge + psk->xGridLocation * KWND_COLWIDTH;
    yKeyTopEdge = yKWNDTopEdge + (psk->yGridLocation - iKeyTop) * KWND_ROWHEIGHT;
    dxKeyWidth = KWND_COLWIDTH * psk->dxColumns;
    ...
    if (!RenderCharacters (pddsTarget, psk->szKeyString, xKeyLeftEdge + 25, yKeyTopEdge, 
        dxKeyWidth - 25, dyKeyHeight, CENTER_JUSTIFY, g_hfontKWND))
    {
        return FALSE;
    }
