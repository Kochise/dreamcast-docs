    ...
    )
{
    ...
    rc.left = 0;
    rc.top = 0;
    rc.right = g_dxCandidateListWidth;
    rc.bottom = KWND_HEIGHT;

    if (lpDDSBack)
    {
        if (g_xCandidateList > (EDITBOX_LEFTEDGE + EDITBOX_WIDTH - g_dxCandidateListWidth))
            g_xCandidateList = (EDITBOX_LEFTEDGE + EDITBOX_WIDTH - g_dxCandidateListWidth);
