...
BOOL                    g_fHWKeybdWasPresent = FALSE;
                   ...
                   UINT uChars)
{
    ...
    PostKeybdMessage(g_hwndEditBox, uVK, nShiftState, uCharNum, &nShiftState, &uChars);
                                      ...
                                      LPVOID                    pButtons)
{
    ...
    if ((pDIDOI->wUsage >= USAGE_FIRST_BUTTON) && (pDIDOI->wUsage <= USAGE_LAST_BUTTON))
        g_rgiButtons[pDIDOI->wUsage - USAGE_FIRST_BUTTON] = (*cButtons)++;
    else
        g_rgiButtons[(*cButtons)] = (*cButtons)++;
...
-------------------------------------------------------------------*/
void InitJoystickToSKBMapping(void)
{
    int             i;

    for (i = 0 ; i < JOYSTICKMAXBUTTONS ; i++) {
        g_rgiButtons[i] = INVALID_INDEX;
    }
...
-------------------------------------------------------------------*/
BOOL CreateDirectInput(HINSTANCE               hinstApp,
                       HWND                    hwndApp,
                       LPDIRECTINPUTDEVICE2*   pdidevice2Joystick)
{
    ...
    hr = g_pdinput->EnumDevices(0, DInputDeviceEnumCallback, pdidevice2Joystick, 
        DIEDFL_ATTACHEDONLY);
    if (hr != DI_OK) {
    ...
    // If we decided the state of hardware keyboard, we have to turn the current
    // keyboard to hardware or software. 
    if (g_fHWKeybdWasPresent != g_fHardwareKeyboardPresent) {
        ...
        g_fHWKeybdWasPresent = g_fHardwareKeyboardPresent;
    }
    ...
    if (*pdidevice2Joystick) {
        ...
        hr = (*pdidevice2Joystick)->SetCooperativeLevel(g_hwndDirectInput, 
        ...
        // button index.
        hr = (*pdidevice2Joystick)->EnumObjects(DIEnumDeviceObjectsProc, &cButtons, DIDFT_BUTTON);
...
void FillPrevButtons(DIJOYSTATE dijoystate)
{
    ...
    g_rgiPrevButtons[JOYSTICK_A] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_A]];
    g_rgiPrevButtons[JOYSTICK_B] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_B]];
    g_rgiPrevButtons[JOYSTICK_X] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_X]];
    g_rgiPrevButtons[JOYSTICK_Y] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_Y]];
    g_rgiPrevButtons[JOYSTICK_LA] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_LA]];
    g_rgiPrevButtons[JOYSTICK_RA] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_RA]];
    g_rgiPrevButtons[JOYSTICK_DA] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_DA]];
    g_rgiPrevButtons[JOYSTICK_UA] = dijoystate.rgbButtons[g_rgiButtons[JOYSTICK_UA]];
...
BOOL CheckForHardwareKeyboard(void)
{
    ...
    if ((GetTickCount() - dwLastEnum) > 500) {
        CreateDirectInput(g_hInstance, g_hwndAPP, &g_pdidevice2JoystickApp);
