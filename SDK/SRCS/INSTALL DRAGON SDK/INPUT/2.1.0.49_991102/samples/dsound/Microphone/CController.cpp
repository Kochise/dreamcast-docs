...
Function:
    ...
    CController::MicrophoneExists

Description:

    This is called when an Microphone device is discovered to be attached to
    the controller.
...
-------------------------------------------------------------------*/
void
CController::MicrophoneExists(GUID guidMicrophone)
{    
    if (m_pMicrophone == NULL)
    {
        m_pMicrophone = new CMicrophone(guidMicrophone);
        if (m_pMicrophone == NULL)
            return;
        
        if (m_pMicrophone->Initialize() == FALSE)
        {
            CMicrophone *pMicrophoneTemp = m_pMicrophone;

            m_pMicrophone = NULL;
            delete pMicrophoneTemp;
            OutputDebugString(TEXT("Error initializing Microphone\r\n"));
