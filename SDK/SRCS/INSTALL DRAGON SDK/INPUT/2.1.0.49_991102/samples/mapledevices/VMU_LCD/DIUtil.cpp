...
Abstract:
   ...
   This file contains DirectInput functionality for the VMU_LCD sample.
...
BOOL CALLBACK LCDEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext);
...
InitDirectInput()
{
    ...
    // Check to see if there are any LCDs currently attached.  If so, then the LCD will be
    // attached to the appropriate port's controller object.
    MapleEnumerateDevices(MDT_LCD, LCDEnumProc, NULL, 0);
...
CheckNewDevices()
{
    ...
    if (WaitForSingleObject(g_hNewDeviceEvent, 0) != WAIT_TIMEOUT)
    {
        ...
        // Enumerate all of the LCD Maple devices
        MapleEnumerateDevices(MDT_LCD, LCDEnumProc, NULL, 0);
    }
}
...
-------------------------------------------------------------------*/
BOOL CALLBACK
LCDEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext)
{
    DWORD dwPort = pmdi->dwPort;
        
    if (pmdi->devType == MDT_LCD && g_rgpcont[dwPort] != NULL)
    {
        DebugOutput(TEXT("Device found on Port %d -- \"%s\"\r\n"), dwPort, 
            pmdi->tszProductName);

        // If the controller's plug is on the top and the VMU's plug is also on 
        // the top, then the LCD is upside down.  If the controller's plug is 
        // on the bottom and the VMU's plug is on the top, then the LCD is 
        // rightside up.  Note that this code assumes that neither plug is on
        // the side of the device.        
        g_rgpcont[dwPort]->m_fInvertedVMU = (DIPROP_EXTRACT_EXPDIRECTION(g_rgpcont[dwPort]->m_nDirection, pmdi->dwDevNum) == pmdi->dwDirection);
        g_rgpcont[dwPort]->LCDExists(pmdi->guidDevice);
    }
