...
public:
    ...
    // Called when an Flash device is discovered to be attached to the controller
    void FlashExists(GUID guidFlashm, int iConn);

    // Get a pointer to the flash device on the specified connection
    CFlash *GetFlash(int nFlash)    { return m_pflash[nFlash]; }
...
private:
    ...
    // The Controller's flash devices
    CFlash *m_pflash[2];
};    
