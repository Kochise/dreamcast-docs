...
CController::FlashExists(GUID guidFlash, int iConn)
{
    ...
    if (m_pflash[0] == NULL)
    {
        // This is the first flash device found
        m_pflash[0] = new CFlash(guidFlash, iConn);
        pflash = m_pflash[0];
    }
    else
    {
        // This is the second flas device found
        m_pflash[1] = new CFlash(guidFlash, iConn);
        pflash = m_pflash[1];
    }
    ...
    if (pflash)
    {
        ...
        if (pflash->Initialize() == FALSE)
        {
            CFlash *pFlashTemp = pflash;
