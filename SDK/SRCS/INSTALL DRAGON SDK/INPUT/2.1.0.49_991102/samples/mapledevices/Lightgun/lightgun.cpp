...
LPDIRECTDRAWSURFACE g_pddsSplash;                  // Contains button bitmaps 

// ++++ Local Variables +++++++++++++++++++++++++++++++++++++++++++++
TCHAR     g_tszAppName[] = TEXT("Lightgun");        // The App's Name
...
RenderSceneToBackBuffer()
{
    ...
    for (iPort = 0; iPort < g_nPorts; iPort++)
        OutputControllerState(iPort, g_rgpcont[iPort]);
...
AppInit(HINSTANCE hPrev, int nCmdShow)
{
    ...
    g_pddsSplash = LoadBitmapToDDS(TEXT("Splash"));
    if (!g_pddsSplash)
        return FALSE;
