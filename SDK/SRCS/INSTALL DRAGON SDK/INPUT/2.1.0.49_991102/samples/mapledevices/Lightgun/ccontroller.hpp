...
public:
    ...
    // Lightgun-related functionality
    BOOL IsLightgun()                {return (m_plightgun != NULL);      };

    // Get a pointer to the lightgun
    CLightgun *GetLightgun()         {return m_plightgun;                };

    // Called when a Lightgun is discovered to be attached to the controller
    void LightgunExists(GUID guidLightgun);
    ...
    // The Controller's Lightgun device (if any)
    CLightgun *m_plightgun;
