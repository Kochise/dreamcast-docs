...
Abstract:
   ...
   This file contains DirectInput functionality for the VMU_Timer sample.
...
BOOL CALLBACK TimerEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext);
...
InitDirectInput()
{
    ...
    // Check to see if there are any Timers currently attached.  If so, then the Timer will be
    // attached to the appropriate port's controller object.
    MapleEnumerateDevices(MDT_TIMER, TimerEnumProc, NULL, 0);
...
-------------------------------------------------------------------*/
BOOL CALLBACK
DIEnumProc(LPCDIDEVICEINSTANCE pdidi, LPVOID context)
{
    UINT iPort;
...
-------------------------------------------------------------------*/
BOOL CALLBACK
TimerEnumProc(LPCMAPLEDEVICEINSTANCE pmdi, LPVOID pvContext)
{
    DWORD dwPort = pmdi->dwPort;
    if (pmdi->devType == MDT_TIMER && g_rgpcont[dwPort] != NULL)
        g_rgpcont[dwPort]->TimerExists(pmdi->guidDevice);
