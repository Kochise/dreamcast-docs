...
Function:
    ...
    CController::VibPackExists

Description:

    This is called when an VibPack device is discovered to be attached to
    the controller.
...
-------------------------------------------------------------------*/
void
CController::VibPackExists(GUID guidVibPack)
{    
    if (m_pvibpack == NULL)
    {
        m_pvibpack = new CVibPack(guidVibPack);
        if (m_pvibpack == NULL)
            return;
        
        if (m_pvibpack->Initialize() == FALSE)
        {
            CVibPack *pVibPackTemp = m_pvibpack;

            m_pvibpack = NULL;
            delete pVibPackTemp;
            OutputDebugString(TEXT("Error initializing VibPack\r\n"));
...
Function:
    ...
    CController::SetVibPack

Description:

    Sets the controller's current vibpack pointer
...
-------------------------------------------------------------------*/
void
CController::SetVibPack(CVibPack *pvibpack)
{
    if (pvibpack == NULL && m_pvibpack != NULL)
    {
        // Release the existing vibpack
        delete m_pvibpack;
    }
