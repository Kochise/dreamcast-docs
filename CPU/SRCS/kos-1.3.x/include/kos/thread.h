...
typedef struct kthread {
	...
	/* Process shell we belong to */
	struct kprocess	* pshell;
...
/* New thread function; given a routine address, it will create a
   ...
   returns, the thread will exit. Returns the new thread id. If pshell
   is non-NULL, the thread will be created in the named shell; otherwise
   it will be created in the local one. */
kthread_t *thd_create(void (*routine)(void *param), void *param);
kthread_t *_thd_create(struct kprocess * pshell, void (*routine)(void *param), void *param);
