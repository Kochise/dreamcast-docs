...
/* This is the number of distinct file descriptors each process gets
   to use. These are private to a process. */
...
/* Takes a handle in the "current" process and duplicates it into the
   target process. This is good for passing back handles between processes
   when necessary (or taking over someone else's stdin/stdout, etc). If newfd
   is non-zero, a dup2-style operation will be performed. The new target
   process handle is returned (useless in the "current" proc). */
struct kprocess;
file_t fs_dup_into(file_t fd, struct kprocess * target, file_t newfd);

/* This is inside the kernel only. These are called by the libcrt shims
   so we can get the process ID of the caller. */
file_t	_fs_open(struct kprocess * proc, const char *fn, int mode);
void	_fs_close(struct kprocess * proc, file_t fd);
ssize_t	_fs_read(struct kprocess * proc, file_t hnd, void *buffer, size_t cnt);
ssize_t	_fs_write(struct kprocess * proc, file_t hnd, const void *buffer, size_t cnt);
off_t	_fs_seek(struct kprocess * proc, file_t hnd, off_t offset, int whence);
off_t	_fs_tell(struct kprocess * proc, file_t hnd);
size_t	_fs_total(struct kprocess * proc, file_t hnd);
dirent_t* _fs_readdir(struct kprocess * proc, file_t hnd);
int	_fs_ioctl(struct kprocess * proc, file_t hnd, void *data, size_t size);
void*	_fs_mmap(struct kprocess * proc, file_t hnd);
...
file_t	_fs_dup_into(struct kprocess * proc, file_t fd, struct kprocess * target, file_t newfd);
file_t	_fs_open_handle(struct kprocess * proc, vfs_handler_t * vfs, void * hnd);
...
/* These two functions are used to reveal "internal" info about a file
   ...
   VFS-sponsored file descriptors (e.g., TCP/IP bind, connect, etc). These
   are not meant to be used for "general consumption", so the native
   versions pass the target process to look in. */
vfs_handler_t * fs_get_handler(struct kprocess * proc, file_t fd);
void * fs_get_handle(struct kprocess * proc, file_t fd);

/* This function duplicates the file descriptor table from one process
   to another, taking care of reference counting and such. Any non-NULL
   file handles will be deref'd, so make sure the table is all NULL'd
   out to begin with. */
int fs_fdtbl_copy(struct kprocess * src, struct kprocess * dst);

/* Takes a defunct process and closes all of its open files. */
int fs_fdtbl_destroy(struct kprocess * proc);
