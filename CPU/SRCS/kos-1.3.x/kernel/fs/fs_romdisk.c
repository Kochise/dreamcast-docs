...
typedef struct rd_image {
	...
	struct kprocess		* owner;	/* Who owns the memory? */
...
int _fs_romdisk_mount(struct kprocess * owner, const char * mountpoint, const uint8 *img) {
...
int fs_romdisk_unmount_proc(struct kprocess * proc) {
	...
	n = LIST_FIRST(&romdisks);
	while (n) {
		nn = LIST_NEXT(n, list_ent);
		if (n->owner == proc) {
			/* Remove it from the mount list */
			LIST_REMOVE(n, list_ent);
