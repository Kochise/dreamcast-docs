...
/* Creates a pty pair */
int _fs_pty_create(kprocess_t * proc, char * buffer, int maxbuflen, file_t * master_out, file_t * slave_out) {
	...
	*slave_out = _fs_open(proc, sname, O_RDWR);
	if (boot) {
		/* Get the slave channel setup first, and dup it across
		   our stdin, stdout, and stderr. */
		_fs_dup2(proc, *slave_out, 1);
		_fs_dup2(proc, *slave_out, 2);
	}
	*master_out = _fs_open(proc, mname, O_RDWR);
...
int fs_pty_init() {
	...
	_fs_pty_create(_local_process, NULL, 0, &cm, &cs);

	/* Close the master end, we want dbgio by default */
	_fs_close(_local_process, cm);

	_fs_pty_create(_local_process, NULL, 0, &tm, &ts);
	_fs_close(_local_process, tm);
	_fs_close(_local_process, ts);
