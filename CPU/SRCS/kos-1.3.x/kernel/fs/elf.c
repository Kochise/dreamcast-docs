...
int elf_load(const char * fn, kprocess_t * proc, elf_prog_t * out) {
	uint8			*img, *imgout;
	int			imgpages, imgoutpages;
	int			sz, i, j, sect; 
	...
	sz = fs_total(fd);
	imgpages = roundpages(sz);
	DBG(("Loading ELF file of size %d\n", sz));

	/* img = malloc(sz); */
	img = (uint8 *)mm_palloc(0, imgpages, proc);
	if (img == NULL) {
		dbglog(DBG_ERROR, "elf_load: can't allocate %d pages for ELF load\n", imgpages);
		fs_close(fd);
		return -1;
	}
	fs_read(fd, img, sz);
	...
	imgoutpages = roundpages(sz);
	out->data = imgout = (void *)mm_palloc(0, imgoutpages, proc);
	if (out->data == NULL) {
		dbglog(DBG_ERROR, "elf_load: can't allocate %d pages for ELF program data\n", imgoutpages);
		goto error1;
	}
	out->size = imgoutpages * PAGESIZE;
	vma = (uint32)imgout;
	for (i=0; i<hdr->shnum; i++) {
		if (shdrs[i].flags & SHF_ALLOC) {
	...
	/* Look for the program entry points and deal with that */
	{
		...
		if (mainsym < 0) {
			dbglog(DBG_ERROR, "elf_load: ELF contains no start()\n");
			goto error3;
		}
...
error3:
	mm_pfree((ptr_t)out->data, imgoutpages);
...
void elf_free(elf_prog_t *prog) {
	mm_pfree((ptr_t)prog->data, prog->size/PAGESIZE);
}
