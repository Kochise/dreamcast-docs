...
CVSID("$Id: thread.c,v 1.28 2003/08/02 09:16:18 bardtx Exp $");
...
int thd_pslist(int (*pf)(const char *fmt, ...)) {
	...
	pf("addr\t\tpid\ttid\tprio\tflags\twait_timeout\tstate     name\n");
...
kthread_t *_thd_create(kprocess_t * pshell, void (*routine)(void *param), void *param) {
	kthread_t	*nt;
	tid_t		tid;
	uint32		params[2];
	int		oldirq = 0;
	...
	if (tid >= 0) {
		...
		if (nt != NULL) {
			...
			/* Create a new thread stack. Note: the _kernel_ owns the thread stack space. */
			/* XXX: Check failure */
			/* nt->stack = (uint32*)malloc(THD_STACK_SIZE); */
			assert( (THD_STACK_SIZE % PAGESIZE) == 0 );
			if (routine != NULL) {
				nt->stack = (uint32 *)mm_palloc(0, THD_STACK_SIZE / PAGESIZE, kernel_process);
...
int thd_destroy(kthread_t *thd) {
	...
	/* Free its stack */
	/* free(thd->stack); */
	if (thd->stack && thd->stack_size)
		mm_pfree((ptr_t)thd->stack, THD_STACK_SIZE / PAGESIZE);
...
void thd_shutdown() {
	...
	while (n1 != NULL) {
		...
		mm_pfree((ptr_t)n1->stack, THD_STACK_SIZE / PAGESIZE);
		free(n1);
		n1 = n2;
	}
