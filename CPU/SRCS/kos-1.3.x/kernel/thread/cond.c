...
CVSID("$Id: cond.c,v 1.10 2003/07/31 00:40:30 bardtx Exp $");
...
/* Allocate a new condvar; the condvar will be assigned
   ...
   will also die. */
condvar_t *_cond_create(struct kprocess * owner) {
	condvar_t	*cv;
	int		old = 0;
...
void cond_freeall(struct kprocess * proc) {
	...
	assert( arch_valid_address(proc) );

	old = irq_disable();

	cv = LIST_FIRST(&cond_list);
	while (cv) {
		ncv = LIST_NEXT(cv, g_list);
