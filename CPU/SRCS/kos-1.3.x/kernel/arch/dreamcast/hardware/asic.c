...
CVSID("$Id: asic.c,v 1.4 2003/07/31 00:42:00 bardtx Exp $");
...
static kprocess_t * asic_evt_handlers_procs[0x4][0x20];

/* Set a handler, or remove a handler */
int _asic_evt_set_handler(kprocess_t * proc, uint32 code, asic_evt_handler hnd) {
	uint32 evtreg, evt;
	int old;
	...
	asic_evt_handlers_procs[evtreg][evt] = proc;
	irq_restore(old);
...
void asic_remove_process(kprocess_t * proc) {
	int old;
	int i, j;
	...
	for (i=0; i<4; i++) {
		for (j=0; j<0x20; j++) {
			if (asic_evt_handlers_procs[i][j] == proc) {
				asic_evt_handlers[i][j] = NULL;
				asic_evt_handlers_procs[i][j] = NULL;
				asic_evt_disable((i << 8) | j, ASIC_IRQ9);
				asic_evt_disable((i << 8) | j, ASIC_IRQB);
				asic_evt_disable((i << 8) | j, ASIC_IRQD);
			}
		}
	}
