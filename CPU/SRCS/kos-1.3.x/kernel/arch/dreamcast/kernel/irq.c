...
CVSID("$Id: irq.c,v 1.15 2003/07/31 00:43:53 bardtx Exp $");
...
static kprocess_t * irq_handlers_procs[0x100];
static irq_handler trapa_handlers[0x100];
static kprocess_t * trapa_handlers_procs[0x100];
...
int _irq_set_handler(kprocess_t * proc, irq_t code, irq_handler hnd) {
	int old;
...
int _irq_set_global_handler(kprocess_t * proc, irq_handler hnd) {
	int old;
...
int _trapa_set_handler(kprocess_t * proc, irq_t code, irq_handler hnd) {
	int old;
	...
	trapa_handlers_procs[code] = proc;
...
void irq_dump_regs(int code, int evt) {
	...
	if (thd_current) {
		dbglog(DBG_DEAD, "Died in thread %d\n", thd_current->tid);
		if (thd_current->pshell)
			dbglog(DBG_DEAD, "Which is in process %d (base %p)\n", thd_current->pshell->pid, thd_current->pshell->image.data);
	}
...
void irq_remove_process(struct kprocess * proc) {
	...
	for (i=0; i<0x100; i++) {
		...
		if (trapa_handlers_procs[i] == proc) {
			trapa_handlers[i] = NULL;
			trapa_handlers_procs[i] = NULL;
		}
	}
...
int irq_init() {
	...
	memset(trapa_handlers_procs, 0, sizeof(trapa_handlers_procs));
	irq_hnd_global = NULL;
	irq_hnd_global_proc = NULL;
