...
void dcln_cmd_sendbinq(ip_header_t * ip, udp_header_t * udp, command_t * command)
{
    ...
    unsigned int bytes_thistime, bytes_rounded;
	...
	bytes_rounded = (bytes_thistime + 1) & ~1;
	
	response->address = htonl((unsigned int)ptr);
	memcpy(response->data, ptr, bytes_thistime);
	response->size = htonl(bytes_rounded);
	dcln_make_ip(ntohl(ip->src), ntohl(ip->dest), UDP_H_LEN + COMMAND_LEN + bytes_rounded, 17, (ip_header_t *)(dcln_pkt_buf + ETHER_H_LEN));
	dcln_make_udp(ntohs(udp->src), ntohs(udp->dest),(unsigned char *) response, COMMAND_LEN + bytes_rounded, (ip_header_t *)(dcln_pkt_buf + ETHER_H_LEN), (udp_header_t *)(dcln_pkt_buf + ETHER_H_LEN + IP_H_LEN));
	dcln_tx(dcln_pkt_buf, ETHER_H_LEN + IP_H_LEN + UDP_H_LEN + COMMAND_LEN + bytes_rounded);
	ptr += bytes_thistime;
    }
