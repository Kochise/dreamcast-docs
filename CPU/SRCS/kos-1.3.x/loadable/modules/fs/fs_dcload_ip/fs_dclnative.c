...
CVSID("$Id: fs_dclnative.c,v 1.4 2003/07/31 00:55:22 bardtx Exp $");
...
static int get_tool_ip(int argc, char **argv) {
#if 0
	hostinfo_t hi;
	...
	if (argc < 3) {
		printf("usage: fs_dcload_ip.elf <our ip> <host ip> <host port>\n");
		printf(" IPs should be in hex format (e.g., 10.3.2.5 = 0a030205\n");
		return -1;
	}

	dcln_our_ip = strtoul(argv[1], 0, 16);
	dcln_tool_ip = strtoul(argv[2], 0, 16);
	dcln_tool_port = strtoul(argv[3], 0, 10);

	/* FIXME: Ain't gonna work for anyone but Dan just yet :) */
	memcpy(dcln_tool_mac, "\x0\x48\x54\x12\x60\x93", 6);
...
ssize_t dclnative_read(void * h, void *buf, size_t cnt) {
    uint32 hnd = (uint32)h;
    ssize_t ret = -1;
...
ssize_t dclnative_write(void * h, const void *buf, size_t cnt) {
    uint32 hnd = (uint32)h;
    ssize_t ret = -1;
...
off_t dclnative_seek(void * h, off_t offset, int whence) {
    uint32 hnd = (uint32)h;
    off_t ret = -1;
...
dirent_t *dclnative_readdir(void * h) {
    uint32 hnd = (uint32)h;
    dirent_t *rv = NULL;
...
/*****************************************************************/
/* Our output thread. We'll hook the master kernel pty and redirect
   any output to the dcload stuff. */
void stdout_thread(void *p) {
	int fd, c;
	char buf[256];

	printf("fs_dclnative: stdout thread starting\n");

	fd = fs_open("/pty/ma00", O_RDWR);
	if (fd < 0) {
		printf("fs_dclnative: warning: can't open /pty/ma00\n");
		return;
	}

	/* Block until we have something to write out */
	for ( ; ; ) {
		c = fs_read(fd, buf, 256);
		if (c <= 0)
			break;
	...
	printf("fs_dclnative: stdout thread quitting\n");
}
...
int module_init(int argc, char **argv) {
	...
	printf("fs_dclnative: initializing\n");
	...
	if (!outdev) {
		/* Shouldn't happen... */
		printf("fs_dclnative_init: couldn't find an output device!\n");
		return -1;
	}
#ifdef DEBUG
...
//#else
	thd_sleep(1000);
	dclnative_printk("Hello, dc-load-ip!\n");
	// old_printk = dbgio_set_printk(dclnative_printk);
//#endif

	/* Start up our stdout drain thread */
	// thd_create(stdout_thread, NULL);
