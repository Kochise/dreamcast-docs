...
static int sock_for_fd(kprocess_t * proc, int fd) {
	int s;

	// Verify that it's actually one of ours
	if (fs_get_handler(proc, fd) != &socketvfs) {
		errno = EBADF;
		return -1;
	}

	// Get the socket #
	s = (int)fs_get_handle(proc, fd); s--;
	if (s < 0) {
		errno = EBADF;
		return -1;
	}
...
int _lwip_socket(kprocess_t * proc, int domain, int type, int protocol) {
	int s, fd;
	...
	fd = _fs_open_handle(proc, &socketvfs, (void *)(s+1));
	if (fd < 0) {
		sock_close(s);
	}
...
int _lwip_close(kprocess_t * proc, int s) {
	_fs_close(proc, s);
	return 0;
}

int _lwip_connect(kprocess_t * proc, int s, struct sockaddr *name, socklen_t namelen) {
	sockfd_t	* fd;
	struct ip_addr	ip;
	int		port, rv = 0;

	s = sock_for_fd(proc, s);
	if (s < 0)
		return -1;
	fd = fds + s;
...
int _lwip_bind(kprocess_t * proc, int s, struct sockaddr *name, socklen_t namelen) {
	sockfd_t	* fd;
	struct ip_addr	ip;
	int		port, rv = 0;
...
int _lwip_listen(kprocess_t * proc, int s, int backlog) {
	struct tcp_pcb	* npcb;
	sockfd_t	* fd;
	int		rv = 0, i;
...
int _lwip_accept(kprocess_t * proc, int s, struct sockaddr *addr, socklen_t *addrlen) {
	sockfd_t	* fd;
	int		rv = 0, i, ns;
	...
	// fd for this socket.
	rv = _fs_open_handle(proc, &socketvfs, (void *)(ns + 1));
	if (rv < 0) {
		close_common(ns);
	}
...
int _lwip_recv(kprocess_t * proc, int s, void *mem, int len, unsigned int flags) {
	s = sock_for_fd(proc, s);
	if (s < 0)
		return -1;
...
int _lwip_send(kprocess_t * proc, int s, const void *mem, int size, unsigned int flags) {
	s = sock_for_fd(proc, s);
	if (s < 0)
		return -1;
...
int _lwip_read(kprocess_t * proc, int s, void *mem, int len) {
	return _lwip_recv(proc, s, mem, len, 0);
}

int _lwip_write(kprocess_t * proc, int s, const void *mem, int len) {
	return _lwip_send(proc, s, mem, len, 0);
}
...
static int select_poll(kprocess_t * proc, int maxfdp1, fd_set *readset, fd_set *writeset, fd_set *exceptset) {
	int i, cnt, s;
	fd_set nr, nw;
...
int _lwip_select(kprocess_t * proc, int maxfdp1, fd_set *readset, fd_set *writeset, fd_set *exceptset,
	struct timeval *timeout)
{
	...
	do {
		// Poll everything
		rv = select_poll(proc, maxfdp1, readset, writeset, exceptset);
...
int _lwip_ioctl(kprocess_t * proc, int s, long cmd, void *argp) {
	errno = ENOSYS;
	return -1;
}
