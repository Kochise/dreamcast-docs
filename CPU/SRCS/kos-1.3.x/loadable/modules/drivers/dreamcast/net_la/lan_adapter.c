...
CVSID("$Id: lan_adapter.c,v 1.2 2003/07/15 07:55:01 bardtx Exp $");
...
int la_init() {
	...
	if (la_if.if_detect(&la_if) < 0) {
		printf("net_la: can't detect lan adapter\n");
		return -1;
	}
	if (la_if.if_init(&la_if) < 0) {
		printf("net_la: can't init lan adapter\n");
		return -1;
	}
	if (la_if.if_start(&la_if) < 0) {
		printf("net_la: can't start lan adapter\n");
		return -1;
	}
...
int la_shutdown() {
	...
	la_if.if_stop(&la_if);
	la_if.if_shutdown(&la_if);
	return 0;
}


int module_init(int argc, char **argv) {
	printf("net_la: initializing\n");
	if (la_init() < 0)
		return -1;
	printf("net_la: done initializing\n");
	return 0;
}

int module_shutdown() {
	printf("net_la: exiting\n");
	net_unreg_device(&la_if);
	if (la_shutdown() < 0)
		return -1;
	return 0;
