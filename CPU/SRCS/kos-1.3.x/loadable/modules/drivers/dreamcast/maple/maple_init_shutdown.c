...
CVSID("$Id: maple_init_shutdown.c,v 1.1 2003/06/19 04:21:00 bardtx Exp $");
...
static void _memalign(int align, int size, uint8 **about, uint8 **ubout) {
	/* Alloc a buffer a little too big */
	ptr_t buf = (ptr_t)malloc(size + align);
	*ubout = (uint8 *)buf;
...
#if MAPLE_DMA_DEBUG
	_memalign(32, MAPLE_DMA_SIZE + 1024, &maple_state.dma_buffer, &maple_state.dma_buffer_unaligned);
	// maple_state.dma_buffer = memalign(32, MAPLE_DMA_SIZE + 1024);
#else
	_memalign(32, MAPLE_DMA_SIZE, &maple_state.dma_buffer, &maple_state.dma_buffer_unaligned);
...
void maple_hw_shutdown() {
	...
	if (maple_state.dma_buffer != NULL) {
		...
		ptr = (uint32)maple_state.dma_buffer_unaligned;
#if MAPLE_DMA_DEBUG
		ptr -= 512;
#endif
