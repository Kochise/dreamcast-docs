...
/* Enable to allow extra debugging checks */
#define DEBUG 1

/* KOS specific things */
#define USE_MALLOC_LOCK
#define MORECORE _kos_morecore
...
/* Fool it into not including any mmap() headers... we emulate mmap using
   mm_palloc. */
#define LACKS_FCNTL_H
#define LACKS_SYS_MMAN_H
#define MAP_ANONYMOUS 1

/* KOS-specific morecore implementation. This thing is large enough that
   it gets broken out into its own module. */
...
#define PROT_READ 1
#define PROT_WRITE 2
static void *mmap(void *ptr, size_t size, int prot, int flags, int fd, off_t offset) {
	ptr_t alloced;
	...
	/* Call down to get the pages */
	alloced = mm_palloc(0, size / PAGESIZE, _local_process);
	/* printf("%s: mapping %d pages for malloc at %p\n", _local_process->label, size / PAGESIZE, (void *)alloced); */
...
static int munmap(void *ptr, size_t size) {
	ptr_t alloced = (ptr_t)ptr;
	/* Check for unsupported things */
	assert( (alloced % PAGESIZE) == 0 );
	assert( (size % PAGESIZE) == 0 );

	/* Call down to free the pages */
	/* printf("%s: unmapping %d pages for malloc at %p\n", _local_process->label, size / PAGESIZE, (void *)alloced); */
	mm_pfree(alloced, size / PAGESIZE);

	return 0;
}

/* realloc is a bear unless we provide this as well */
static void * mremap(void * oldaddr, size_t old_size, size_t new_size, unsigned long flags) {
	ptr_t oldp = (ptr_t)oldaddr, newp;
	int pgdiff;

	assert( (oldp % PAGESIZE) == 0 );
	assert( (old_size % PAGESIZE) == 0 );
	assert( (new_size % PAGESIZE) == 0 );
	assert( flags );

	pgdiff = (new_size - old_size) / PAGESIZE;
	if (pgdiff == 0) {
		printf("%s: remap request with same size (%d) at %p\n",
			_local_process->argv[0], new_size, oldaddr);
		return oldaddr;
	}

	/* Shrinking always succeeds */
	if (pgdiff < 0) {
		/* printf("%s: remap shrinkage at %p by %d pages\n",
			_local_process->label, oldaddr, -pgdiff); */
		mm_pfree(oldp + new_size, -pgdiff);
		return oldaddr;
	} else {
		/* printf("%s: remap growage at %p by %d pages\n",
			_local_process->label, oldaddr, pgdiff); */

		/* Try to grow the block in place */
		newp = mm_palloc(oldp + old_size, pgdiff, _local_process);
		if (newp == oldp + old_size) {
			/* Got it */
			return (void *)oldp;
		}

		/* No good -- free that block, allocate a new one, and copy over the
		   old contents. */
		mm_pfree(newp, pgdiff);
		newp = mm_palloc(0, new_size/PAGESIZE, _local_process);
		// printf("  (unable to extend block -- alloc'd new block at %p)\n", (void*)newp);
		if (!newp)
			return MAP_FAILED;
		memcpy((void *)newp, (void *)oldp, old_size);
		mm_pfree(oldp, old_size/PAGESIZE);
