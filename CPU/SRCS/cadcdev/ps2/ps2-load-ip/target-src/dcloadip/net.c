/* ps2-load-ip
   ...
   $Id: net.c,v 1.3 2002/11/03 06:57:13 bardtx Exp $
*/
...
void net_init() {
	int i;
	for (i=0; i<broadcast[i]; i++)
		broadcast[i] = 0xff;

	set_ip();
	gs_printf(LCD_LOADMSG_FG, "Pre-configured IP appears to be %s\n", PS2_IP);
}
...
void process_broadcast(unsigned char *pkt, int len)
{
	...
	if (!memcmp(arp_header->proto_target, &ip, 4)) { /* for us */
	    ...
	    memcpy(ether_header->src, smap_get_mac(), 6);
	    ...
	    memcpy(arp_header->hw_sender, smap_get_mac(), 6);
	    /* transmit */
	    smap_send(pkt, ETHER_H_LEN + ARP_H_LEN);
	}
    }
}
...
void process_icmp(ether_header_t *ether, ip_header_t *ip, icmp_header_t *icmp)
{
  ...
  if (icmp->type == 8) { /* echo request */
    ...
    /* transmit */
    smap_send((uint8 *)ether, ETHER_H_LEN + ntohs(ip->length));
  }
}
...
void process_udp(ether_header_t *ether, ip_header_t *ip, udp_header_t *udp)
{
  ...
  if (!memcmp(command->id, CMD_REBOOT, 4)) {
      cmd_reboot(ether, ip, udp, command);
  }
}
...
void process_pkt(unsigned char *pkt, int len)
{
    ...
    if (!memcmp(ether_header->dest, smap_get_mac(), 6)) {
