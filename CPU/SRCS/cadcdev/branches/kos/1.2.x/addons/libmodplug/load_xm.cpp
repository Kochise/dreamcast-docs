...
//--------------------------------------------------------------
{
	...
	memcpy(&dwHdrSize, lpStream+60, 4);
	dwHdrSize=bswapLE32(dwHdrSize);
	//dwHdrSize = bswapLE32(*((DWORD *)(lpStream+60)));
	memcpy(&norders, lpStream+64, 2);
	norders=bswapLE16(norders);
	//norders = bswapLE16(*((WORD *)(lpStream+64)));
	if ((!norders) || (norders > MAX_ORDERS)) return FALSE;
	memcpy(&restartpos, lpStream+66, 2);
	restartpos=bswapLE16(restartpos);
	//restartpos = bswapLE16(*((WORD *)(lpStream+66)));
	memcpy(&channels, lpStream+68, 2);
	channels=bswapLE16(channels);
	...
	memcpy(&patterns, lpStream+70, 2);
	patterns=bswapLE16(patterns);
	//patterns = bswapLE16(*((WORD *)(lpStream+70)));
	if (patterns > 256) patterns = 256;
	memcpy(&instruments, lpStream+72, 2);
	instruments=bswapLE16(instruments);
	...
	memcpy(&defspeed, lpStream+76, 2);
	defspeed=bswapLE16(defspeed);
	//defspeed = bswapLE16(*((WORD *)(lpStream+76)));
	memcpy(&deftempo, lpStream+78, 2);
	deftempo=bswapLE16(deftempo);
	...
	for (UINT ipat=0; ipat<patterns; ipat++)
	{
		...
		memcpy(&dwSize, lpStream+dwMemPos, 4);
		dwSize=bswapLE32(dwSize);
		...
		memcpy(&rows, lpStream+dwMemPos+5, 2);
		rows=bswapLE16(rows);
		//rows = bswapLE16(*((WORD *)(&lpStream[dwMemPos+5])));
		if ((!rows) || (rows > 256)) rows = 64;
		memcpy(&packsize, lpStream+dwMemPos+7, 2);
		packsize=bswapLE16(packsize);
	...
	for (UINT iIns=1; iIns<=instruments; iIns++)
	{
		...
		memcpy(&pih_size, &pih->size, 4);
		if (dwMemPos + bswapLE32(pih_size) > dwMemLength) return TRUE;
		...
		memcpy(&pih_samples, &pih->samples, 1);
		if ((nsamples = pih_samples) > 0)
		{
			...
			dwMemPos += bswapLE32(pih_size);
		} else
		{
			if (bswapLE32(pih_size)) dwMemPos += bswapLE32(pih_size);
	...
	DWORD tmp;
	memcpy(&tmp, lpStream+dwMemPos, 4);
	tmp=bswapLE32(tmp);
	if ((dwMemPos + 8 < dwMemLength) && (tmp == 0x74786574))
	{
	...
	memcpy(&tmp, lpStream+dwMemPos, 4);
	tmp=bswapLE32(tmp);
	if ((dwMemPos + 8 < dwMemLength) && (tmp == 0x4944494D))
	{
		UINT len = *((DWORD *)(lpStream+dwMemPos+4));
		dwMemPos += 8;
	...
	memcpy(&tmp, lpStream+dwMemPos, 4);
	tmp=bswapLE32(tmp);
	if ((dwMemPos + 8 < dwMemLength) && (tmp == 0x4d414e50))
	{
		UINT len = *((DWORD *)(lpStream+dwMemPos+4));
		dwMemPos += 8;
	...
	memcpy(&tmp, lpStream+dwMemPos, 4);
	tmp=bswapLE32(tmp);
	if ((dwMemPos + 8 < dwMemLength) && (tmp == 0x4d414e43))
	{
		UINT len = *((DWORD *)(lpStream+dwMemPos+4));
		dwMemPos += 8;
