...
static int gaps_init() {
	...
	if (gaps_detect() < 0) {
		printf("bba: no ethernet card found\n");
		return -1;
	}
	...
	if (!(g2_read_32(GAPS_BASE + 0x1418) & 1)) {
		printf("bba: GAPS PCI controller not responding; giving up!\n");
		return -1;
	}
...
static int bba_hw_init() {
	...
	if (g2_read_8(NIC(RT_CHIPCMD)) & RT_CMD_RESET) {
		printf("bba: timed out on reset #1\n");
		return -1;
	}
	...
	if (g2_read_8(NIC(RT_CHIPCMD)) & RT_CMD_RESET) {
		printf("bba: timed out on reset #2\n");
		return -1;
	}
	...
	printf("bba: MAC Address is %02x:%02x:%02x:%02x:%02x:%02x\n",
...
static int bba_rx() {
	...
	while (!(g2_read_8(NIC(RT_CHIPCMD)) & 1)) {
		...
		if (rx_size == 0xfff0) {
			printf("bba: early receive triggered\n");
			break;
		}
		
		if (!(rx_status & 1)) {
			printf("bba: frame receive error, status is %08lx; skipping\n", rx_status);
			rx_reset();
			break;
		}
		...
		if ((rx_status & 1) && (pkt_size <= 1514)) {
			...
			printf("bba: bogus packet receive detected; skipping packet\n");
			rx_reset();
			break;
		}
...
static void bba_irq_hnd(uint32 code) {
	...
	if (intr & RT_INT_LINK_CHANGE) {
		...
		if (!link_initial) {
			...
			printf("bba: initial link change, redoing auto-neg\n");
		}
		...
		if (bmsr & RT_MII_LINK) {
			...
			printf("bba: link stable\n");

			// The link is back.
			link_stable = 1;
		} else {
			if (link_initial)
				printf("bba: link lost\n");
	...
	if (intr & RT_INT_RXBUF_OVERFLOW) {
		printf("bba: RX overrun\n");
		rx_reset();
		hnd = 1;
	}
	/* if (!hnd) {
		printf("bba: spurious interrupt, status is %04x\n", intr);
	} */
}
...
static int bba_if_start(netif_t *self) {
	...
	if (!link_stable) {
		printf("bba: timed out waiting for link to stabilize\n");
		return -1;
	}
